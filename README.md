...
This application is optimized for high-concurrency production deployment. For scaling to 10,000+ concurrent users:

### Quick Checklist

1. **Install scaling dependencies**: `pip install -r requirements.txt` (adds Redis, gevent)
2. **Set up Redis**: Required for rate limiting and caching in production
3. **Configure environment**: Copy `.env.production.example` and populate real secrets
4. **Deploy with Gunicorn**: Use the supplied `gunicorn.conf.py` for cooperative workers
5. **Validate performance**: Run synthetic load using `locust -f loadtests/locustfile.py`

### Key Features for Scale

- **Cooperative Workers**: Gevent-based async processing
- **Connection Pooling**: Optimized SQLAlchemy pools for high concurrency
- **Redis Caching**: Billing data cached to reduce database load
- **Rate Limiting**: Redis-backed rate limiting for production traffic
- **Load Testing**: Comprehensive Locust scenarios included

### Production Deployment

See [`docs/operations/SCALING_RUNBOOK.md`](docs/operations/SCALING_RUNBOOK.md) for complete deployment instructions, performance tuning, and monitoring guidance.