...
| `ingredients/compiler.py` | Orchestrates the iterative build: grabs the next term, locks it, calls the AI worker, validates, writes `outputs/ingredients/<slug>.json`, and updates lookup files. |
...
- By default, relies solely on the built-in exemplar list plus the AI librarian. If you want to ingest legacy seed files, pass `--ingest-seeds --seed-root <dir>`. The `outputs/` folder is always ignored.
...
2. **Initialize the processing queue.**
   ```bash
   python -m data_builder.ingredients.compiler --terms-file data_builder/data_sources/terms.json --max-ingredients 0
   ```
...
- Each ingredient lives in its own file, e.g., `outputs/ingredients/lavender.json` containing the parent ingredient and all of its items/forms.
...
data_builder/
  data_sources/
    vocab/                # curated seed vocab + rules
    terms.json            # optional legacy seed list
  outputs/
...
| **EU CosIng** | – | `COSING_CSV_PATH` (defaults to `data_builder/data_sources/cosing.csv`) | Download the CosIng Excel/CSV export and drop it into the data_sources folder. |
...
Any file path variables default to the `data_builder/data_sources/` directory, so you can just place the CSV/JSON there without setting environment variables. To override, point the corresponding `_PATH` variable at another location (e.g., `COSING_CSV_PATH=/mnt/data/cosing.csv`).
