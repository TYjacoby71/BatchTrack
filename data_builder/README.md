...
# Data Builder Toolkit - Standalone Operation

This directory contains a completely isolated ingredient data builder system, separate from the main BatchTrack application.
...
### Run the normalizer (processes source data):
```bash
cd data_builder
python run_normalizer.py
```

### Enqueue normalized terms:
```bash
cd data_builder
python run_enqueue.py --from=db
```

### Run the compiler:
```bash
cd data_builder
python run_compiler.py --max-ingredients=50
```

### View the portal (ingredient browser):
```bash
cd data_builder
python run_portal.py --port=5055
```

### Run scrapers:
```bash
cd data_builder
python scrapers/tgsc_scraper.py
```

## Database

The data builder maintains its own SQLite database at `data_builder/ingredients/compiler_state.db`, completely separate from BatchTrack's PostgreSQL database.

## Components

| Module | Purpose |
| --- | --- |
| `ingredients/term_collector.py` | Finds canonical ingredient terms and physical forms |
| `ingredients/database_manager.py` | Manages the SQLite queue (`compiler_state.db`) |
| `ingredients/ai_worker.py` | Processes ingredients via OpenAI API |
| `ingredients/compiler.py` | Orchestrates the iterative build process |
| `ingredients/portal.py` | Web interface for browsing compiled ingredients |
| `scrapers/tgsc_scraper.py` | Scrapes ingredient data from external sources |
