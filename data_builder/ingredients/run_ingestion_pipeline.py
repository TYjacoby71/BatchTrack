...
def run() -> None:
    ...
    # 1) Merge catalog identities (CAS/EC/INCI) for cross-source enrichment.
    ...
    # 2) Ingest per-row items (variation/form parsing + provenance). Do NOT write normalized_terms here;
    # canonical terms are derived from the merged catalog later.
    inserted_items, _inserted_terms = ingest_source_items.ingest_sources(
        cosing_path=cosing_path,
        tgsc_path=tgsc_path,
        limit=None,
        sample_size=None,
