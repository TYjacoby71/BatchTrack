...
def _pick_canonical_term_for_cluster(
    ...
    *,
    ...
    # Count TGSC base common candidates (variation-stripped).
    counts: dict[str, int] = {}
    for it in items:
        cn_raw = _clean(getattr(it, "common_name", None))
        if not cn_raw:
            continue
        display = cn_raw
        variation, physical_form = extract_variation_and_physical_form(display)
        base = derive_term_from_common_name(cn_raw, variation, physical_form)
        base = _clean_base_common_name(base)
        if not base:
            continue
        key = base.lower()
        counts[key] = counts.get(key, 0) + 1

    # Consensus selection: pick the most frequent; require 2+ occurrences, OR 1 occurrence + binomial present.
    best_common = ""
    best_common_count = 0
    for cand, c in sorted(counts.items(), key=lambda kv: (-kv[1], len(kv[0]), kv[0])):
        best_common = cand
        best_common_count = c
        break

    if best_common and (best_common_count >= 2 or (best_common_count == 1 and cluster_bot)):
        term = _soft_title(best_common)
        term = _disambiguate_term(term, cluster_bot or best_common, used_terms)
        return term, cluster_bot, "tgsc_base_common_name"

    # Fallback 1: binomial identity (single).
    if cluster_bot:
    ...
    # Fallback 2: derive from INCI if present.
    for it in items:
        inci = _clean(getattr(it, "inci_name", None))
        if inci:
            term = derive_definition_term(inci)
            term = _disambiguate_term(term, _botanical_key_from_catalog_item(it) or term.lower(), used_terms)
            return term, _botanical_key_from_catalog_item(it), "inci_name"

    # Last resort: any stable display name.
    any_name = _clean(getattr(items[0], "common_name", None)) or _clean(getattr(items[0], "inci_name", None))
