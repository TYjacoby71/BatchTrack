...
def repair(*, consolidate: bool, dry_run: bool) -> dict[str, int]:
    ...
    with database_manager.get_session() as session:
        ...
        # - Butter: keep as physical form only (not a variation) for plant-derived fats.
        # - Form tokens like Oil/Hydrosol/Powder: can remain as a variation label, but should be bypassed
        #   so item names render cleanly (Base Oil vs Base (Oil)).
        form_token_bypass = {"Oil", "Hydrosol", "Powder", "Wax", "Resin", "Gum", "Gel", "Paste"}
        # Fix plant butter identity: variation should be blank when physical_form is Butter.
        src_rows = (
        ...
        # Ensure variation_bypass is set for form-token variations like Oil/Oil, Hydrosol/Hydrosol.
        token_rows = (
            session.query(database_manager.SourceItem)
            .filter(
                database_manager.SourceItem.derived_variation.in_(list(form_token_bypass)),
        ...
        for r in token_rows:
            ...
            if old != 1:
                if not dry_run:
                    r.variation_bypass = 1
                    r.variation_bypass_reason = "form_token"
                stats["source_items_bypass_updated"] += 1

        # 2) Fix merged_item_forms identity fields.
        # Only fix Butter/Butter at the merged identity level (keep other tokens as-is, rely on bypass for UI).
        mif_rows = (
        ...
        # 3) Consolidate duplicates created by normalization (Butter only).
