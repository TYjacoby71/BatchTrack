...
@contextmanager
def _safe_txn():
    """Run enclosed DDL in a SAVEPOINT so failures don't abort the migration.

    Postgres marks the entire transaction as failed after any error. Many of the
    best-effort schema tweaks below can legitimately fail on some databases
    (e.g., due to existing data or provider differences). We isolate each group
    inside a SAVEPOINT so we can roll back just that group and continue.
    """
    bind = op.get_bind()
    try:
        txn = bind.begin_nested()
    except Exception:
        # If nested transactions are not supported, execute without savepoint.
        txn = None
    try:
        yield
        if txn is not None:
        ...
        if txn is not None:
            txn.rollback()
        # Swallow to allow the rest of the migration to proceed
        pass
