"""Remove container_size from batch containers

Revision ID: 0c98c20f2100
Revises: 112dc7f2bfc1
Create Date: 2025-06-27 18:06:50.732205

"""
from alembic import op
import sqlalchemy as sa


# revision identifiers, used by Alembic.
revision = '0c98c20f2100'
down_revision = '112dc7f2bfc1'
branch_labels = None
depends_on = None


def upgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    with op.batch_alter_table('batch_container', schema=None) as batch_op:
        batch_op.alter_column('organization_id',
               existing_type=sa.INTEGER(),
               nullable=False)
        batch_op.create_index(batch_op.f('ix_batch_container_organization_id'), ['organization_id'], unique=False)
        batch_op.drop_column('container_size')

    with op.batch_alter_table('extra_batch_container', schema=None) as batch_op:
        batch_op.drop_column('container_size')

    # ### end Alembic commands ###


def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    with op.batch_alter_table('extra_batch_container', schema=None) as batch_op:
        batch_op.add_column(sa.Column('container_size', sa.VARCHAR(length=32), nullable=False))

    with op.batch_alter_table('batch_container', schema=None) as batch_op:
        batch_op.add_column(sa.Column('container_size', sa.VARCHAR(length=32), nullable=False))
        batch_op.drop_index(batch_op.f('ix_batch_container_organization_id'))
        batch_op.alter_column('organization_id',
               existing_type=sa.INTEGER(),
               nullable=True)

    # ### end Alembic commands ###
