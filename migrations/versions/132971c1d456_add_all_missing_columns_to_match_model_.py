...
def upgrade():
    # ### TEMPORARILY DISABLE ALL CONSTRAINTS FOR MIGRATION ###
    # This allows us to add columns and constraints in any order
    ...
    try:
        ...
        # ### RE-ENABLE ALL CONSTRAINTS AFTER MIGRATION ###
        # This restores normal constraint checking
        connection.execute(sa.text("SET session_replication_role = DEFAULT;"))

    # Now add constraints AFTER re-enabling constraint checking
    # Foreign key constraints for product_sku
    with op.batch_alter_table('product_sku', schema=None) as batch_op:
        try:
            batch_op.create_foreign_key('fk_product_sku_variant', 'product_variant', ['variant_id'], ['id'])
        except:
            pass  # Skip if constraint already exists or table doesn't exist
        try:
    ...
    # Unique constraints for product_sku (only if values exist and are unique)
    with op.batch_alter_table('product_sku', schema=None) as batch_op:
        try:
            batch_op.create_unique_constraint('unique_barcode', ['barcode'])
        except:
            pass  # Skip if constraint would fail due to duplicate values
        try:
            batch_op.create_unique_constraint('unique_upc', ['upc'])
        except:
            pass

    # Foreign key constraints for product_sku_history
    with op.batch_alter_table('product_sku_history', schema=None) as batch_op:
        try:
