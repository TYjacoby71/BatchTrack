...
def safe_drop_foreign_key(table_name, column_name):
    """Safely drop a foreign key constraint associated with a column if it exists."""
    print(f"      Dropping foreign key for column '{column_name}' from table '{table_name}'...")
    try:
        bind = op.get_bind()
        inspector = inspect(bind)
        ...
        if fk_to_drop:
            ...
            print(f"      ✅ Foreign key '{fk_to_drop}' dropped from table '{table_name}'.")
        else:
            print(f"      ⚠️ Foreign key for column '{column_name}' does not exist on table '{table_name}', skipping drop.")
    except Exception as e:
        print(f"      ⚠️ Could not drop foreign key for column '{column_name}' on table '{table_name}': {e}")
...
def upgrade():
    ...
    # Handle global_item table modifications with safe helpers
    safe_drop_index('global_item', 'ix_global_item_is_archived')
    safe_drop_foreign_key('global_item', 'archived_by')
    safe_add_column('global_item', 'archived_by', sa.Column('archived_by', sa.Integer(), nullable=True))

    # Add foreign key constraint for archived_by column if it doesn't exist
    try:
        bind = op.get_bind()
        inspector = inspect(bind)
        fk_constraints = inspector.get_foreign_keys('global_item')

        # Check if foreign key already exists
        fk_exists = any(
            'archived_by' in fk['constrained_columns'] and fk['referred_table'] == 'user'
            for fk in fk_constraints
        )
        ...
        print(f"      ⚠️ Could not create foreign key constraint for archived_by: {e}")
