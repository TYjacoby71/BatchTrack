...
def safe_drop_table(table_name):
    ...
    if result.fetchone()[0]:
        ...
        print(f"   ⚠️  Table {table_name} doesn't exist, skipping drop")
...
def safe_drop_column(table_name, column_name):
    ...
    if column_exists(table_name, column_name):
        ...
        print(f"   ℹ️  Column {column_name} doesn't exist in {table_name}")


def safe_create_table_if_not_exists(table_name, *args, **kwargs):
...
def upgrade():
    ...
    bind = op.get_bind()
    inspector = inspect(bind)
    # Introspect existing FKs on batch to avoid dropping non-existent constraints in batch mode (SQLite)
    try:
    ...
    safe_create_table_if_not_exists('developer_role_permission',
    ...
    safe_create_table_if_not_exists('subscription_tier_permission',
    ...
    safe_drop_table('pricing_snapshot')
    safe_drop_table('billing_snapshot')
