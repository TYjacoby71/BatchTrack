...
down_revision = 'final_consolidation_2025'
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    # Create organization_stats table only if it doesn't exist
    ...
    try:
        ...
        if '_alembic_tmp_role' in existing_tables:
    ...
    with op.batch_alter_table('user', schema=None) as batch_op:
        ...
        if 'subscription_class' in user_columns:
            try:
                batch_op.drop_column('subscription_class')
            except:
                pass

        if 'is_owner' in user_columns:
            try:
...
def downgrade():
    ...
    # Drop statistics tables only if they exist
    if 'user_stats' in existing_tables:
        op.drop_table('user_stats')
    if 'organization_stats' in existing_tables:
