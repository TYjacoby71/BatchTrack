...
def seed_consumables_from_files(selected_files):
    """Seed consumable items from JSON files"""
    ...
    base_dir = os.path.join(
        os.path.dirname(__file__), "globallist", "consumables", "categories"
    )
    ...
    for filename in selected_files:
        ...
        try:
            ...
            logger.warning("Suppressed exception fallback at app/seeders/seed_consumables.py:32", exc_info=True)
            print(f"  ⚠️  Error loading {filename}: {e}")
            continue

        cat_name = category_data.get("category_name", "").strip()
        print(f"\n📁 Processing consumables category: {cat_name}")
        ...
        for item_data in category_data.get("items", []):
            ...
            existing_item = GlobalItem.query.filter_by(
                name=name, item_type="consumable"
            ).first()
...
if __name__ == "__main__":
    ...
    with app.app_context():
        # Get available consumables files
        base_dir = os.path.join(
            os.path.dirname(__file__), "globallist", "consumables", "categories"
        )
        available_files = []
        ...
        if not available_files:
            print("No consumables JSON files found")
            sys.exit(1)

        print("Available consumables files:")
        ...
        items_created = seed_consumables_from_files(selected_files)

        try:
            db.session.commit()
            print("\n🎉 Consumables Seeding Complete!")
            print(f"📊 Items created: {items_created}")
        except Exception as e:
            logger.warning("Suppressed exception fallback at app/seeders/seed_consumables.py:130", exc_info=True)
            db.session.rollback()
