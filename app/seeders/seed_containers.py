...
def _get_or_create_ingredient(name: str):
    base_name = (name or '').strip() or 'Container'
...
def seed_containers_from_files(selected_files):
    ...
    base_dir = os.path.join(os.path.dirname(__file__), 'globallist', 'containers', 'categories')
    ...
    for filename in selected_files:
        ...
        for item_data in category_data.get('items', []):
            ...
            physical_form_name = item_data.get('physical_form') or item_data.get('container_type') or 'Container'
...
if __name__ == '__main__':
    ...
    with app.app_context():
        ...
        if existing_containers > 0:
            print(f"Found {existing_containers} existing containers, generating attribute files...")
