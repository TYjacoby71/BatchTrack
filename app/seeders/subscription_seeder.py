...
from ..models import Organization, Permission, SubscriptionTier, db
...
def safe_db_operation(operation_name, operation_func):
    ...
    try:
        ...
        logger.warning(
            "Suppressed exception fallback at app/seeders/subscription_seeder.py:31",
            exc_info=True,
        )
        db.session.rollback()
...
def create_free_tier():
    ...
    def _create():
        ...
        if not free_tier:
            ...
            free_permissions = Permission.query.filter(
                Permission.name.in_(
                    [
                        "recipes.view",
                        "recipes.create",
...
def create_solo_tier():
    ...
    def _create():
        ...
        if not solo_tier:
            ...
            solo_permissions = Permission.query.filter(
                Permission.name.in_(
                    [
                        "dashboard.view",
                        "inventory.view",
                        ...
                        "ai.recipe_optimization",
                        "ai.demand_forecasting",
                        "ai.quality_insights",
                    ]
                )
            ).all()
...
def create_enterprise_tier():
    ...
    def _create():
        enterprise_tier = SubscriptionTier.query.filter_by(
        ...
        if not enterprise_tier:
            ...
            print(
                f"   - Enterprise tier created with {len(enterprise_permissions)} permissions"
            )
        else:
...
def seed_subscription_tiers():
    ...
    if not current_app:
        raise RuntimeError(
            "seed_subscription_tiers() must be called within Flask application context"
        )
    ...
    for tier_name, creator_func in tier_creators:
        try:
            ...
            logger.warning(
                "Suppressed exception fallback at app/seeders/subscription_seeder.py:373",
                exc_info=True,
            )
    ...
    try:
        ...
        logger.warning(
            "Suppressed exception fallback at app/seeders/subscription_seeder.py:381",
            exc_info=True,
        )
...
def migrate_existing_organizations():
    ...
    if not current_app:
        raise RuntimeError(
            "migrate_existing_organizations() must be called within Flask application context"
        )
...
def seed_subscriptions():
    ...
    if not current_app:
        raise RuntimeError(
            "seed_subscriptions() must be called within Flask application context"
        )
