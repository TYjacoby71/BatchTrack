...
def _get_or_create_ingredient(name: str):
    base_name = (name or '').strip() or 'Packaging'
...
def seed_packaging_from_files(selected_files):
    ...
    base_dir = os.path.join(os.path.dirname(__file__), 'globallist', 'packaging', 'categories')
    ...
    for filename in selected_files:
        ...
        for item_data in category_data.get('items', []):
            ...
            physical_form_name = item_data.get('physical_form') or item_data.get('container_type') or 'Packaging'
