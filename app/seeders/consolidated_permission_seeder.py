...
from ..models import DeveloperPermission, Permission, Role, db
...
def load_consolidated_permissions():
    ...
    if not os.path.exists(json_path):
        raise FileNotFoundError(
            f"Consolidated permissions file not found at: {json_path}"
        )
    ...
    try:
        ...
        if (
            "organization_permissions" not in data
            or "developer_permissions" not in data
        ):
            raise ValueError(
                "Invalid permissions file structure - missing required sections"
            )
    ...
    try:
        ...
        for category_key, category_data in org_permissions.items():
            if (
                ...
                or "permissions" not in category_data
            ):
            ...
            for perm_data in permissions:
                ...
                if existing:
                    existing.description = perm_data.get(
        ...
        db.session.commit()
        print(
            f"   ✅ Organization permissions: {permissions_created} created, {permissions_updated} updated"
        )
    ...
    try:
        ...
        for category_key, category_data in dev_permissions.items():
            ...
            for perm_data in permissions:
                ...
                existing = DeveloperPermission.query.filter_by(
                    name=perm_data["name"]
                ).first()
        ...
        db.session.commit()
        print(
            f"   ✅ Developer permissions: {permissions_created} created, {permissions_updated} updated"
        )
...
def seed_developer_roles():
    ...
    db.session.commit()
    print("   ✅ Developer roles: 3 roles created/updated")
...
def update_organization_owner_role():
    ...
    if org_owner_role:
        ...
        customer_permissions = Permission.query.filter(Permission.is_active).all()
        ...
        print(
            f"✅ Updated organization owner role with {len(customer_permissions)} permissions"
        )
    else:
        print(
            "⚠️  organization_owner role not found - it should be created by seed_organization_roles()"
        )
...
def cleanup_old_permissions():
    ...
    old_dev_perms = DeveloperPermission.query.filter(
        ~DeveloperPermission.name.in_(allowed_dev_names)
    ).all()
...
def seed_organization_roles():
    ...
    print("   ✅ Organization system roles: 1 role created/updated")
...
def seed_consolidated_permissions():
    ...
    if not current_app:
        raise RuntimeError(
            "seed_consolidated_permissions() must be called within Flask application context"
        )
    ...
    print(
        f"   ✅ Permissions complete: {org_count} org permissions, {dev_count} dev permissions"
    )
