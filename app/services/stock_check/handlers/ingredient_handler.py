...
class IngredientHandler(BaseInventoryHandler):
    ...
    def check_availability(
        ...
        ingredient = InventoryItem.query.filter_by(
            id=request.item_id, organization_id=organization_id
        ).first()
        ...
        effective_tracking_enabled = (
            bool(getattr(ingredient, "is_tracked", True)) and org_tracks_quantities
        )
            ...
            try:
                ...
                if not conversion_result.get("success"):
                    conversion_details = {
                        ...
                        "requires_attention": conversion_result.get(
                            "requires_attention", False
                        ),
                        "error_message": conversion_result.get("error_data", {}).get(
                    ...
                    if conversion_result.get("drawer_payload"):
                        conversion_details["drawer_payload"] = conversion_result.get(
                            "drawer_payload"
                        )
                    return StockCheckResult(
                        ...
                        formatted_needed=self._format_quantity_display(
                            request.quantity_needed, recipe_unit
                        ),
                ...
                conversion_details = {
                    "conversion_type": conversion_result.get(
                ...
                logger.exception(
                    "Untracked stock check conversion failed for item %s", ingredient.id
                )
                return StockCheckResult(
                    item_id=ingredient.id,
                    item_name=ingredient.name,
        ...
        logger.debug(
            f"Ingredient {ingredient.name}: {total_available} {stock_unit} available, need {request.quantity_needed} {recipe_unit}"
        )
        ...
        try:
            ...
            if conversion_result["success"]:
                ...
                available_in_recipe_units = (
                    stock_to_recipe_result["converted_value"]
                    if stock_to_recipe_result["success"]
                ...
                return StockCheckResult(
                    ...
                    formatted_available=self._format_quantity_display(
                        available_in_recipe_units, recipe_unit
                    ),
                    conversion_details=conversion_details,
                )
            else:
            ...
            logger.exception(
                f"An unexpected error occurred during stock check for item {ingredient.id}"
            )
            status = StockStatus.ERROR
            conversion_details = {
                "error_type": "system_error",
    ...
    def _determine_status_with_thresholds(
        self, available: float, needed: float, ingredient: InventoryItem
    ) -> StockStatus:
        ...
        if available >= needed:
            ...
            if (
                ...
                and available <= ingredient.low_stock_threshold
            ):
