...
class BatchStatisticsService:
    ...
    @staticmethod
    def create_from_production_plan(
        batch_id: int, production_plan: Dict[str, Any]
    ) -> Optional[BatchStats]:
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    "Suppressed exception fallback at app/services/statistics/_batch_stats.py:39",
                    exc_info=True,
                )
                pass
            ...
            db.session.commit()
            logger.info(
                f"Created batch stats for batch {batch_id} with {planned_efficiency}% efficiency"
            )
            return batch_stats
    ...
    @staticmethod
    def complete_batch(
        batch_id: int, completion_data: Dict[str, Any]
    ) -> Optional[BatchStats]:
        ...
        try:
            ...
            # Update batch stats
            batch_stats.update_actual_data(
                actual_efficiency, actual_yield, actual_costs
            )
            ...
            batch = db.session.get(Batch, batch_id)
            freshness_summary = (
                FreshnessService.compute_batch_freshness(batch) if batch else None
            )

            # Attach lightweight metrics to stats for downstream reporting (not persisted fields)
            overall_freshness = (
                getattr(freshness_summary, "overall_freshness_percent", None)
    ...
    def get_batch_efficiency_report(batch_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "spoilage_cost": batch_stats.ingredient_spoilage_cost
                + batch_stats.product_spoilage_cost,
