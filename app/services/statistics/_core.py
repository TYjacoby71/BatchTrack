...
from ...models.statistics import OrganizationLeaderboardStats, OrganizationStats
...
class StatisticsService:
    ...
    @staticmethod
    def record_planned_efficiency(
        recipe_id: int,
        planned_efficiency: float,
        planned_yield: Dict[str, Any],
        planned_costs: Dict[str, Any] = None,
    ):
        ...
        try:
            ...
            RecipeStatisticsService.increment_batch_planned(
                recipe_id, current_user.organization_id
            )

            logger.info(
                f"Recorded planned efficiency: {planned_efficiency}% for recipe {recipe_id}"
            )
    ...
    def record_batch_completion(batch_id: int, efficiency_data: Dict[str, Any]):
        ...
        try:
            ...
            batch_stats = BatchStatisticsService.complete_batch(
                batch_id, efficiency_data
            )
            if batch_stats:
                # Update related recipe and organization stats
                StatisticsService._update_recipe_stats(batch_stats.recipe_id)
                StatisticsService._update_organization_stats(
                    batch_stats.organization_id
                )
    ...
    def _update_organization_stats(organization_id: int):
        ...
        top_tester = (
            ...
            .group_by(Recipe.created_by)
            .order_by(sa.desc("test_count"))
            .first()
        )
