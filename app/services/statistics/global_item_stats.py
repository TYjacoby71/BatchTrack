...
from app.models import InventoryItem, InventoryLot, RecipeIngredient, db
...
class GlobalItemStatsService:
    ...
    def get_rollup(global_item_id: int):
        ...
        item_ids_subq = (
            ...
            .filter(InventoryItem.global_item_id == global_item_id)
            .subquery()
        )

        # Adoption counts
        organizations = (
            db.session.query(func.count(func.distinct(InventoryItem.organization_id)))
            .filter(
        ...
        recipes = (
            db.session.query(func.count(func.distinct(RecipeIngredient.recipe_id)))
            .join(InventoryItem, RecipeIngredient.inventory_item_id == InventoryItem.id)
            .filter(
                InventoryItem.global_item_id == global_item_id,
        ...
        avg_cost = (
            ...
            .join(item_ids_subq, InventoryLot.inventory_item_id == item_ids_subq.c.id)
            .scalar()
        )
        ...
        age_days = (
            db.session.query(
                func.avg(
                    func.extract("epoch", func.now() - InventoryLot.received_date)
                    / 86400.0
                )
            )
            .join(item_ids_subq, InventoryLot.inventory_item_id == item_ids_subq.c.id)
            .filter(InventoryLot.remaining_quantity_base > 0)
            .scalar()
        )

        # Average expired amounts (quantity and time to expiry) based on lots that are expired with remaining qty
        expired_qty = (
            db.session.query(func.avg(InventoryLot.remaining_quantity))
            .join(item_ids_subq, InventoryLot.inventory_item_id == item_ids_subq.c.id)
            .filter(
        ...
        avg_days_past = (
            db.session.query(
                func.avg(
                    func.extract("epoch", func.now() - InventoryLot.expiration_date)
                    / 86400.0
                )
            )
            .join(item_ids_subq, InventoryLot.inventory_item_id == item_ids_subq.c.id)
        ...
        return {
            ...
            "average_expired_quantity": (
                float(expired_qty) if expired_qty is not None else None
            ),
            "average_days_past_expiration": (
                float(avg_days_past) if avg_days_past is not None else None
            ),
        }
    ...
    def get_cost_distribution(global_item_id: int) -> dict:
        ...
        non_outliers = [
            v
            for v in costs
            if (lower_fence is None or v >= lower_fence)
            and (upper_fence is None or v <= upper_fence)
        ]
        outliers_low = [v for v in costs if lower_fence is not None and v < lower_fence]
        outliers_high = [
            v for v in costs if upper_fence is not None and v > upper_fence
        ]
        ...
        if non_outliers and (high is not None) and (low is not None) and high > low:
            ...
            for i in range(bins):
                ...
                count = len(
                    [
                        v
                        for v in non_outliers
                        if (v >= start and (v < end or (i == bins - 1 and v <= end)))
                    ]
                )
