...
class RecipeMarketplaceService:
    ...
    def _parse_marketplace_fields(cls, form, *, existing=None) -> Dict[str, Any]:
        ...
        shopify_url = _get("shopify_product_url")
        if shopify_url is None and existing is not None and "shopify_product_url" not in form:
            shopify_url = getattr(existing, "shopify_product_url", None)
        if isinstance(shopify_url, str):
