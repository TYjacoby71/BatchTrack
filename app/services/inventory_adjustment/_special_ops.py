...
from ._fifo_ops import (  # Kept for local use within this file and added deduct_fifo_inventory
...
def handle_cost_override(
    ...
    **kwargs,
    ...
    try:
        ...
        logger.info(
            f"COST OVERRIDE: Item {item.id} cost updated from {old_cost} to {cost_override}"
        )
        return (
            True,
            f"Cost updated from {old_cost} to {cost_override} per {item.unit or 'unit'}",
        )
    ...
    **kwargs,
    ...
    try:
        ...
        evt = UnifiedInventoryHistory(
            ...
            notes=(
                notes
                or f"Unit conversion verified: {quantity} {unit} -> {conv['converted_value']} {item.unit}"
            ),
            created_by=created_by,
        ...
        db.session.add(evt)
        logger.info(
            f"UNIT CONVERSION: Verified {quantity} {unit} -> {conv['converted_value']} {item.unit} for item {item.id}"
        )
...
def handle_recount(
    ...
    **kwargs,
    ...
    try:
        ...
        from ._fifo_ops import create_new_fifo_lot, get_item_lots
        ...
        if delta_base == 0:
            ...
            logger.info(
                f"RECOUNT: Deducting {abs_delta} using recount-specific FIFO drainage"
            )
            ...
            if fifo_total_base < abs_delta_base:
                return (
                    False,
                    f"Cannot recount: need to deduct {abs_delta}, but only {total_available} available",
                )
            ...
            for lot in active_lots:
                ...
                event_code = generate_inventory_event_code(
                    change_type, item_id=item.id, code_type="event"
                )
                ...
                deduction_history = UnifiedInventoryHistory(
                    ...
                    notes=f"RECOUNT: Deducted {deduct_from_lot} from lot {lot.fifo_code}"
                ...
                logger.info(
                    f"RECOUNT DEDUCT: Consumed {deduct_from_lot} from lot {lot.id} ({lot.fifo_code})"
                )
            ...
            logger.info(
                f"RECOUNT: Adding {delta} - checking for refillable lots vs overflow"
            )

            # Get existing lots that can be refilled (depleted lots first, then partial lots)
            existing_lots = get_item_lots(
                item.id, active_only=False, order="desc"
            )  # Newest first
            refillable_lots = [
                lot
                for lot in existing_lots
                if int(lot.remaining_quantity_base or 0)
                < int(lot.original_quantity_base or 0)
            ]
            ...
            for lot in refillable_lots:
                ...
                available_capacity_base = int(lot.original_quantity_base or 0) - int(
                    lot.remaining_quantity_base or 0
                )
                ...
                if refill_amount_base > 0:
                    # Refill the lot
                    lot.remaining_quantity_base = int(
                        lot.remaining_quantity_base or 0
                    ) + int(refill_amount_base)
                    sync_lot_quantities_from_base(lot, item)
                    db.session.add(lot)
                    ...
                    refill_history = UnifiedInventoryHistory(
                        ...
                        unit=lot.unit,
                        unit_cost=lot.unit_cost,
                        notes=f"RECOUNT: Refilled {refill_amount} to lot {lot.fifo_code}"
                        + (f" | {notes}" if notes else ""),
                        created_by=created_by,
                        organization_id=item.organization_id,
                        affected_lot_id=lot.id,  # ALWAYS reference the affected lot
                    ...
                    logger.info(
                        f"RECOUNT: Refilled {refill_amount} to lot {lot.id} ({lot.fifo_code})"
                    )
            ...
            if remaining_to_add_base > 0:
                ...
                logger.info(
                    f"RECOUNT: Creating overflow lot for remaining {remaining_to_add}"
                )
            ...
            if remaining_to_add_base > 0:
                summary_parts.append("created overflow lot")

            summary_msg = (
                " and ".join(summary_parts)
                if summary_parts
                else f"processed {delta} inventory"
            )
            logger.info(f"RECOUNT SUCCESS: {summary_msg}")

        # Return success - core will set the absolute quantity using target_quantity
        logger.info(
            f"RECOUNT COMPLETE: Item {item.id} FIFO reconciled for target {target_qty}"
        )
