...
def _universal_additive_handler(
    item,
    quantity,
    quantity_base,
    change_type,
    ...
    **kwargs,
    ...
    try:
        ...
        if not effective_tracking_enabled:
            ...
            success_message = action_messages.get(
                change_type,
                f"{change_type.replace('_', ' ').title()} added {quantity} {unit}",
            )
            return (
                True,
                f"{success_message} (infinite item: quantity unchanged)",
                0.0,
                0,
            )
        ...
        logger.info(
            f"{change_type.upper()} SUCCESS: Will increase item {item.id} by {quantity_delta}"
        )
...
def _handle_lot_creation_operation(
    item,
    quantity,
    quantity_base,
    change_type,
    ...
    try:
        ...
        if (
            quantity is None or quantity_base is None
        ):  # Added check here to handle potential None quantity passed to this function
            return False, f"Invalid quantity: {quantity}", 0
        quantity_delta = float(quantity)
        logger.info(
            f"LOT_CREATION SUCCESS: Will add {quantity_delta} to item {item.id}"
        )

        return (
            True,
            f"{change_type.title()} of {quantity} {unit} completed",
            quantity_delta,
        )
...
def _handle_lot_crediting_operation(
    item,
    quantity,
    quantity_base,
    change_type,
    ...
    **kwargs,
    ...
    logger.info(
        f"LOT_CREDITING: Processing {change_type} credit operation for {quantity} {unit}"
    )
    ...
    try:
        ...
        lots_to_credit = (
            InventoryLot.query.filter(
                and_(
                    ...
                    InventoryLot.remaining_quantity_base
                    < InventoryLot.original_quantity_base,  # Lots that have been consumed from
                )
            )
        ...
        for lot in lots_to_credit:
            ...
            space_available_base = int(lot.original_quantity_base) - int(
                lot.remaining_quantity_base
            )
            ...
            if space_available_base > 0:
                ...
                lot.remaining_quantity_base = int(lot.remaining_quantity_base) + int(
                    credit_amount_base
                )
                ...
                history_record = UnifiedInventoryHistory(
                    ...
                    notes=f"{change_type.title()}: Credited {credit_amount} back to lot {lot.fifo_code}"
                    + (f" | {notes}" if notes else ""),
                    created_by=created_by,
                    organization_id=item.organization_id,
                ...
                logger.info(
                    f"LOT_CREDITING: Credited {credit_amount} back to lot {lot.id} ({lot.fifo_code}), new remaining: {lot.remaining_quantity}"
                )
        ...
        if remaining_to_credit_base > 0:
            ...
            logger.info(
                f"LOT_CREDITING: Creating new lot for overflow {remaining_to_credit} {unit}"
            )
            ...
            if remaining_to_credit is None:
                return (
                    False,
                    f"Invalid quantity for overflow lot: {remaining_to_credit}",
                    0,
                )
            ...
            success, message, overflow_lot_id = create_new_fifo_lot(
                ...
                change_type=change_type,
                unit=unit,
                notes=f"{change_type.title()} overflow: {remaining_to_credit}"
                + (f" | {notes}" if notes else ""),
                cost_per_unit=final_cost,
                created_by=created_by,
                batch_id=batch_id,
        ...
        if lots_credited > 0 and remaining_to_credit_base > 0:
            success_msg = (
                f"Credited to {lots_credited} existing lots and created overflow lot"
            )
        elif lots_credited > 0:
            success_msg = (
                f"Credited back to {lots_credited} existing lots using FIFO order"
            )
        else:
