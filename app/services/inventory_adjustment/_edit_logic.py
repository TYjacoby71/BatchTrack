...
def _drain_lots_for_infinite_mode(
    item: InventoryItem, *, updated_by: int | None = None
) -> int:
    active_lots = (
...
def update_inventory_item(
    item_id: int, form_data: dict, *, updated_by: int | None = None
) -> tuple[bool, str]:
    """
    ...
    try:
        ...
        if is_global_locked:
            ...
            for forbidden_key in ["name", "category_id", "density", "item_density"]:
                if forbidden_key in form_data and str(
                    form_data.get(forbidden_key)
                ).strip() != str(
                    getattr(
                        item, "name" if forbidden_key == "name" else forbidden_key, ""
                    )
                ):
                    return (
                        False,
                        "This item is managed by the global catalog. Identity fields cannot be edited.",
                    )
        ...
        if "unit" in form_data and form_data["unit"] and form_data["unit"] != item.unit:
            ...
            try:
                ...
                return (
                    False,
                    f"Cannot change base unit from {old_unit} to {new_unit}: {str(e)}",
                )
            ...
            if not probe or not probe.get("success"):
                ...
                return (
                    False,
                    f"Cannot change base unit from {old_unit} to {new_unit}: {err_msg or 'Invalid conversion'}",
                )
            ...
            for lot in lots:
                try:
                    ...
                    if (
                        not rem_conv
                        or not rem_conv.get("success")
                        or rem_conv.get("converted_value") is None
                    ):
                        db.session.rollback()
                        return (
                            False,
                            f"Error converting lot #{lot.id} remaining quantity to {new_unit}",
                        )
                    if (
                        not orig_conv
                        or not orig_conv.get("success")
                        or orig_conv.get("converted_value") is None
                    ):
                        db.session.rollback()
                        return (
                            False,
                            f"Error converting lot #{lot.id} original quantity to {new_unit}",
                        )
                    ...
                    return (
                        False,
                        f"Error converting lot #{lot.id} to {new_unit}: {str(e)}",
                    )
            ...
            if total_remaining_base is None:
                try:
                    ...
                    if (
                        not qconv
                        or not qconv.get("success")
                        or qconv.get("converted_value") is None
                    ):
                        db.session.rollback()
                    ...
                    return (
                        False,
                        f"Error converting item quantity to {new_unit}: {str(e)}",
                    )
            else:
        ...
        if not is_global_locked:
            ...
            cleaned_name = (
                ...
                else (str(raw_name).strip() if raw_name else "")
            )
            if cleaned_name:
        ...
        organization = (
            db.session.get(Organization, item.organization_id)
        ...
        if not bool(item.is_tracked):
            ...
            if not anchor_ok:
                return (
                    False,
                    anchor_message or "Failed to initialize infinite anchor lot.",
                )

        if switched_to_infinite:
            # Explicit user-driven toggles require confirmation before draining lots.
            confirmed_drain = (
                _parse_bool_flag(form_data.get("confirm_infinite_drain")) is True
            )
            ...
            drained_total_base = _drain_lots_for_infinite_mode(
        ...
        if "is_perishable" in form_data:
            item.is_perishable = form_data["is_perishable"] in [
                "True",
                "true",
                "1",
                "on",
        ...
        if "shelf_life_days" in form_data:
            try:
                ...
                if old_shelf_life != new_shelf_life:
                    logger.info(
                        f"Updated shelf life for item {item_id} from {old_shelf_life} to {new_shelf_life} days. Existing lots unchanged."
                    )
        ...
        if ("density" in form_data) or ("item_density" in form_data):
            ...
            if is_global_locked:
                return (
                    False,
                    "This item is managed by the global catalog. Density cannot be edited.",
                )
            ...
            try:
                ...
                return (
                    False,
                    "Invalid density value; please provide a numeric g/mL value",
                )
            ...
            for field_name, converter in converters.items():
                if field_name in form_data:
                    ...
                    try:
                        ...
                        return (
                            False,
                            f"Invalid numeric value for {field_name.replace('_', ' ')}",
                        )
