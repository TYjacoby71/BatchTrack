...
from ...models import ConversionLog, CustomUnitMapping
from ...models import InventoryItem as Ingredient
    ...
    def convert_units(
        ...
        if not from_u:
            base_result = {
                ...
                "error_data": {
                    "unit": from_unit,
                    "message": f'Unit "{from_unit}" not found in system',
                },
                "conversion_type": "failed",
                "density_used": None,
                "from": from_unit,
        ...
        if not to_u:
            base_result = {
                ...
                "error_data": {
                    "unit": to_unit,
                    "message": f'Unit "{to_unit}" not found in system',
                },
                "conversion_type": "failed",
                "density_used": None,
                "from": from_unit,
        ...
        def find_conversion_path(start, end, visited=None):
            ...
            if (
                ...
                forward_mappings = CustomUnitMapping.query.filter_by(
                    from_unit=start, organization_id=current_user.organization_id
                ).all()
                reverse_mappings = CustomUnitMapping.query.filter_by(
                    to_unit=start, organization_id=current_user.organization_id
                ).all()
            else:
        ...
        if conversion_path:
            ...
            conversion_type = (
                "custom_compound" if len(conversion_path) > 1 else "custom"
            )
        ...
        # 3. Same-type base conversion (volume → volume, weight → weight)
            ...
            try:
                from_multiplier = (
                    float(from_u.conversion_factor) if from_u.conversion_factor else 1.0
                )
                to_multiplier = (
                    float(to_u.conversion_factor) if to_u.conversion_factor else 1.0
                )
        ...
        # 4. Cross-type: volume ↔ weight
            ...
            if density is None or (isinstance(density, (int, float)) and density <= 0):
                ...
                if drawer_info.get("requires_drawer") and drawer_info.get(
                    "drawer_payload"
                ):
        ...
        result = {
            ...
            "from": from_unit,
            "to": to_unit,
            "requires_attention": conversion_type
            in ["custom", "density"],  # Adjust this based on desired attention flags
        }
    ...
    def validate_density_requirements(from_unit, to_unit, ingredient=None):
        ...
        def _resolve_ingredient(ingredient_ref):
            if ingredient_ref is None:
                return None
            if hasattr(ingredient_ref, "density") or hasattr(
                ingredient_ref, "category"
            ):
        ...
        if {"volume", "weight"}.issubset({from_type, to_type}):
            ...
            if not ingredient_obj:
                return (
                    True,
                    "Ingredient context required for volume ↔ weight conversion",
                )
            ...
            return (
                True,
                f"Density required for {ingredient_name}. Set ingredient density or category.",
            )
