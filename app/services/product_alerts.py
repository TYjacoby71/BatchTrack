...
class ProductAlertService:
    ...
    def get_product_stock_summary() -> Dict:
        ...
        for sku in low_stock_skus:
            if sku.product_name not in low_stock_products:
                low_stock_products[sku.product_name] = []
            low_stock_products[sku.product_name].append(sku)
        
        for sku in out_of_stock_skus:
            if sku.product_name not in out_of_stock_products:
                out_of_stock_products[sku.product_name] = []
            out_of_stock_products[sku.product_name].append(sku)
