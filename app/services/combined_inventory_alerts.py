...
class CombinedInventoryAlertService:
    ...
    def get_expiration_alerts(days_ahead: int = 7) -> Dict:
        ...
        try:
            ...
            # Debug logging
            logging.info(
                f"Expiration alerts debug: expired_fifo={len(expired_fifo_entries)}, expired_products={len(expired_products)}, expiring_fifo={len(expiring_fifo_entries)}, expiring_products={len(expiring_products)}"
            )
            ...
            return {
                ...
                "has_any_expiration_issues": expired_total > 0
                or expiring_soon_total > 0,
            }
    ...
    def get_low_stock_skus():
        ...
        history_exists = (
            ...
            .filter(InventoryHistory.inventory_item_id == InventoryItem.id)
            .exists()
        )
    ...
    def get_unified_stock_summary() -> Dict:
        ...
        out_of_stock_product_items = (
            CombinedInventoryAlertService.get_out_of_stock_skus()
        )
        ...
        return {
            ...
            "affected_products_count": len(
                set(low_stock_products.keys()) | set(out_of_stock_products.keys())
            ),
            # Combined totals
            "total_low_stock_items": len(low_stock_ingredients)
            + len(low_stock_product_items),
            "total_critical_items": len(out_of_stock_product_items),
            "has_any_alerts": len(low_stock_ingredients) > 0
            or len(low_stock_product_items) > 0
            or len(out_of_stock_product_items) > 0,
        }
