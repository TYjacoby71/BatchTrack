...
def scale_recipe(
    recipe_id: int, target_yield: float, yield_unit: str = None
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if not recipe.predicted_yield or recipe.predicted_yield <= 0:
            return {
                "success": False,
                "error": "Recipe has no predicted yield to scale from",
            }
...
def calculate_scaled_ingredients(recipe_id: int, scale_factor: float) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.error(
            f"Error calculating scaled ingredients for recipe {recipe_id}: {e}"
        )
        return {"success": False, "error": str(e)}
