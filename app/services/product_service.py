...
class ProductService:
    ...
    def resolve_bulk_size_label(unit: str | None) -> str:
        ...
        try:
            ...
            unit_row = Unit.query.filter(
                (func.lower(Unit.name) == unit_key)
                | (func.lower(Unit.symbol) == unit_key)
            ).first()
            if not unit_row:
                return "Bulk"
            ...
            logger.warning(
                "Suppressed exception fallback at app/services/product_service.py:42",
                exc_info=True,
            )
            return "Bulk"
    ...
    def get_or_create_sku(
        ...
        try:
            normalized_size_label = (
                "" if size_label is None else str(size_label)
            ).strip()
            ...
            logger.warning(
                "Suppressed exception fallback at app/services/product_service.py:70",
                exc_info=True,
            )
            normalized_size_label = "Bulk"
        ...
        if not product:
            ...
            try:
                ...
                default_cat = ProductCategory.query.filter_by(
                    name="Uncategorized"
                ).first()
                if not default_cat:
                ...
                logger.warning(
                    "Suppressed exception fallback at app/services/product_service.py:98",
                    exc_info=True,
                )
                category_id = None
            product = Product(
        ...
        if not variant:
            variant = ProductVariant(
                ...
                created_by=(
                    getattr(current_user, "id", None) or product.created_by or 1
                ),
            )
        ...
        if is_bulk_label:
            ...
            bulk_candidates = [
                ...
                if ProductService.is_bulk_size_label(
                    getattr(sku_candidate, "size_label", None)
                )
            ]
            sku_candidate_pools = [
                [
                    ...
                    for candidate_pool in sku_candidate_pools:
                        ...
                        for candidate in candidate_pool:
                            ...
                            try:
                                ...
                                logger.warning(
                                    "Suppressed exception fallback at app/services/product_service.py:242",
                                    exc_info=True,
                                )
                                continue
            else:
        ...
        if not sku:
            ...
            try:
                ...
                try:
                    ...
                    logger.warning(
                        "Suppressed exception fallback at app/services/product_service.py:293",
                        exc_info=True,
                    )
                    category = None
                template = (
                    ...
                    base_context.update(
                        {
                            ...
                            for k, v in naming_context.items()
                        }
                    )
                sku_name = SKUNameBuilder.render(template, base_context)
            except Exception:
                logger.warning(
                    "Suppressed exception fallback at app/services/product_service.py:315",
                    exc_info=True,
                )
        ...
        try:
            if naming_context:
                ...
                try:
                    category = (
                        db.session.get(ProductCategory, sku.product.category_id)
                        if getattr(sku.product, "category_id", None)
                        else None
                    )
                except Exception:
                    logger.warning(
                        "Suppressed exception fallback at app/services/product_service.py:373",
                        exc_info=True,
                    )
                    category = None
                template = (
            ...
            logger.warning(
                "Suppressed exception fallback at app/services/product_service.py:394",
                exc_info=True,
            )
            pass
    ...
    def get_product_summary_skus():
        ...
        org_id = (
            getattr(current_user, "organization_id", None) if current_user else None
        )
        if not org_id:
            ...
            groups.append(
                {
                    ...
                    "quantity": (
                        sku.inventory_item.quantity if sku.inventory_item else 0.0
                    ),
                    "unit": sku.unit,
    ...
    def get_low_stock_skus(threshold_multiplier: float = 1.0):
        ...
        return (
            ...
            .filter(
                ...
                <= ProductSKU.low_stock_threshold * threshold_multiplier,
                ProductSKU.is_active,
                ProductSKU.organization_id == current_user.organization_id,
            )
            .all()
    ...
    def get_sku_by_id(sku_id):
        ...
        return ProductSKU.query.filter_by(
            id=sku_id, organization_id=current_user.organization_id
        ).first()
    ...
    def get_product_from_sku(sku_id):
        ...
        return {"product_id": sku.product_id, "sku_id": sku.id}

    @staticmethod
    def quick_add_product(
        product_name: str, variant_name: str = "Base", product_base_unit: str = "oz"
    ):
