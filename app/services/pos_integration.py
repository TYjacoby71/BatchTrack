...
class POSIntegrationService:
        ...
        try:
            ...
            recent_entry = (
                UnifiedInventoryHistory.query.filter_by(
                    inventory_item_id=item_id, change_type="reserved", order_id=order_id
                )
            ...
            source_fifo_id = (
                recent_entry.affected_lot_id
                if recent_entry and getattr(recent_entry, "affected_lot_id", None)
                else None
            )
            ...
            reservation = Reservation(
                ...
                organization_id=(
                    ...
                    else original_item.organization_id
                ),
            )
    ...
    def confirm_sale(order_id: str, notes: str = None) -> Tuple[bool, str]:
        ...
        try:
            ...
            active_reservations = Reservation.query.filter(
                and_(Reservation.order_id == order_id, Reservation.status == "active")
            ).all()
            ...
            for reservation in active_reservations:
                ...
                if not reserved_item:
                    logger.error(
                        f"Reserved item not found for reservation {reservation.id}"
                    )
                    continue
                ...
                if not item:
                    logger.error(
                        f"Inventory item not found for reservation {reservation.id}"
                    )
                    continue
                ...
                if not sale_success:
                    logger.error(
                        f"Failed to process sale adjustment for reservation {reservation.id}"
                    )
        ...
        try:
            ...
            completed_reservations = Reservation.query.filter(
                and_(
                    ...
                    Reservation.status
                    == "converted_to_sale",  # Or potentially 'completed' depending on reservation status logic
                )
            ).all()
            ...
            for reservation in completed_reservations:
                ...
                if not item:
                    logger.error(
                        f"Inventory item not found for return reservation {reservation.id}"
                    )
                    continue
                ...
                if not return_success:
                    logger.error(
                        f"Failed to process return adjustment for reservation {reservation.id}"
                    )
            ...
            return (
                True,
                f"Processed return of {total_returned} units for order {order_id}",
            )
    ...
    def cleanup_expired_reservations() -> int:
        ...
        try:
            ...
            for reservation in expired_reservations:
                ...
                success, _ = POSIntegrationService.release_reservation(
    ...
    def get_active_reservations_for_item(item_id: int) -> List[Reservation]:
        ...
        return Reservation.query.filter(
            and_(Reservation.product_item_id == item_id, Reservation.status == "active")
        ).all()
