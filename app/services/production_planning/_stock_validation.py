...
def validate_recipe_stock(recipe: Recipe, scale: float = 1.0) -> Dict[str, Any]:
    """
    Public method for recipe stock validation
    Returns detailed ingredient status for UI display
    """
    logger.info(f"🔍 STOCK VALIDATION: Validating recipe {recipe.id} at scale {scale}")

    # Use the internal stock check service
    from app.services.stock_check.core import UniversalStockCheckService
    uscs = UniversalStockCheckService()

    # Get stock check results
    ...
    if not result['success']:
        return {
            ...
            'ingredient_issues': [],
            'ingredient_status': [],
            'error': result.get('error', 'Stock validation failed')
        }
    ...
    for item in stock_results:
        status_info = {
            ...
            'status': item['status'],
            'formatted_needed': item['formatted_needed'],
            'formatted_available': item['formatted_available']
        }
        ingredient_status.append(status_info)

        # Check if this affects production capability
        if item['status'] not in ['available', 'OK']:
            can_produce = False
            ingredient_issues.append(status_info)
