...
def calculate_production_requirements(recipe_id: int, scale: float = 1.0) -> Dict[str, Any]:
    """
    Legacy compatibility function for calculating just the requirements.
    Delegates to comprehensive planning but returns simplified format.
    """
    try:
        plan_result = plan_production_comprehensive(
            recipe_id=recipe_id,
            scale=scale,
        ...
        for req in plan_result['ingredient_requirements']:
            ingredients.append({
                'ingredient_id': req['item_id'],
                'ingredient_name': req['item_name'], 
                'base_quantity': req['needed_quantity'] / scale,  # Calculate base from scaled
                'scaled_quantity': req['needed_quantity'],
                'unit': req['unit'],
...
def validate_production_feasibility(recipe_id: int, scale: float = 1.0) -> Dict[str, Any]:
