...
from ..models import Batch, BatchContainer, ExtraBatchContainer, InventoryItem, db
...
class BatchIntegrationService:
    ...
    def consume_ingredients(
        batch: Batch, ingredient_consumption: Dict[str, float]
    ) -> bool:
        ...
        try:
            for item_id, quantity in ingredient_consumption.items():
                ...
                if not result["success"]:
                    logger.error(
                        f"Failed to consume ingredient {item_id} for batch {batch.id}: {result.get('error')}"
                    )
                    return False
    ...
    def finish_batch(batch: Batch, output_sku_id: str, output_quantity: float) -> bool:
        ...
        try:
            ...
            if not result["success"]:
                logger.error(
                    f"Failed to credit output for finished batch {batch.id}: {result.get('error')}"
                )
                return False
    ...
    def get_batch_containers_summary(self, batch_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            containers = BatchContainer.query.filter_by(
                batch_id=batch_id, organization_id=current_user.organization_id
            ).all()
            ...
            for container in containers:
                capacity = (
                    container.inventory_item.capacity or 0
                ) * container.quantity_used
                container_info = {
                    "id": container.id,
                    "name": (
                        container.inventory_item.container_display_name
                        if container.inventory_item
                        else "Unknown"
                    ),
            ...
            for extra_container in extra_containers:
                capacity = (
                    extra_container.inventory_item.capacity or 0
                ) * extra_container.quantity_used
                container_info = {
                    "id": extra_container.id,
                    "name": (
                        extra_container.inventory_item.container_display_name
                        if extra_container.inventory_item
                        else "Unknown"
                    ),
            ...
            return {
                "success": True,
                "data": {"containers": container_data, "summary": summary},
            }
    ...
    def remove_container_from_batch(
        self, batch_id: int, container_id: int
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if not result["success"]:
                return {
                    "success": False,
                    "error": f"Failed to restore inventory: {result.get('error')}",
                }
    ...
    def adjust_batch_container(
        ...
        try:
            ...
            if adjustment_type == "quantity":
                return self._adjust_quantity_via_inventory(
                    container_record, batch_id, adjustment_data
                )
            elif adjustment_type == "replace":
                return self._replace_via_inventory(
                    container_record, batch_id, adjustment_data
                )
            else:
    ...
    def _adjust_quantity_via_inventory(self, container_record, batch_id, data):
        ...
        if quantity_difference != 0:
            ...
            if not result["success"]:
                return {
                    "success": False,
                    "error": f"Inventory adjustment failed: {result.get('error')}",
                }
    ...
    def _replace_via_inventory(self, container_record, batch_id, data):
        ...
        if not result1["success"]:
            return {
                "success": False,
                "error": f"Failed to return old containers: {result1.get('error')}",
            }
        ...
        if not result2["success"]:
            ...
            return {
                "success": False,
                "error": f"Failed to deduct new containers: {result2.get('error')}",
            }
