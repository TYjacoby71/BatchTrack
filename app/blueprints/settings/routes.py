...
def index():
    ...
    is_org_owner = (
        ...
        and current_user.organization.owner
        and current_user.organization.owner.id == current_user.id
    )
    ...
    try:
        ...
        if not user_prefs:
            return jsonify(
                {
                    "success": False,
                    "message": "User preferences not available for this user type",
                }
            )
...
def update_timezone():
    ...
    if timezone and TimezoneUtils.validate_timezone(timezone):
        ...
        flash(
            "Invalid timezone selected. Please choose from the available options.",
            "error",
        )
    return redirect(url_for("settings.index"))
