...
from ...utils.permissions import has_permission, has_tier_permission, require_permission
...
def _extract_stock_issues(stock_items):
    ...
    for item in stock_items or []:
        ...
        raw_category = (
            item.get("category") or item.get("type") or item.get("item_type") or ""
        )
...
def list_batches():
    ...
    try:
        ...
        batch_data = BatchManagementService.prepare_batch_list_data(
            filters, pagination_config
        )

        return render_template(
            "pages/batches/batches_list.html",
...
def view_batch_record(batch_identifier):
    ...
    try:
        ...
        if batch.status == "in_progress":
            print("DEBUG: Redirecting to in_progress view")
            return redirect(
                url_for("batches.view_batch_in_progress", batch_identifier=batch.id)
            )
...
def update_batch_notes(batch_id):
    ...
    try:
        ...
        success, message = BatchService.update_batch_notes_and_tags(
            batch_id, notes, tags
        )
        ...
        if request.is_json:
            if success:
                return jsonify(
                    {"message": message, "redirect": url_for("batches.list_batches")}
                )
            else:
                return jsonify({"error": message}), (
                    500 if "Permission" not in message else 403
                )
...
def view_batch_in_progress(batch_identifier):
    ...
    try:
        ...
        if batch.status != "in_progress":
            ...
            return redirect(
                url_for("batches.view_batch_record", batch_identifier=batch_identifier)
            )
        ...
        return render_template(
            "pages/batches/batch_in_progress.html",
            batch=batch,
            timers=timers,
...
def get_available_ingredients_for_batch(recipe_id):
    ...
    try:
        ...
        ingredients_data, error = (
            BatchManagementService.get_available_ingredients_for_batch(recipe_id, scale)
        )
...
def api_start_batch():
    ...
    try:
        ...
        try:
            ...
            if not stock_result.get("success"):
                error_msg = (
                    ...
                    or "Unable to verify inventory for this recipe."
                )
            ...
            return (
                jsonify(
                    {
                        "success": False,
                        "message": "Inventory validation failed. Please try again.",
                    }
                ),
                500,
            )
        ...
        skip_ingredient_ids = [
            ...
            and (issue.get("category") or "").lower() == "ingredient"
        ]
        skip_consumable_ids = [
            issue["item_id"]
            for issue in stock_issues
            if issue.get("item_id")
            and (issue.get("category") or "").lower() == "consumable"
        ]
        forced_note = (
            _build_forced_start_note(stock_issues)
            if force_start and stock_issues
            else None
        )
