...
def _parse_adjustment_result(result):
    ...
    if not isinstance(result, tuple) or len(result) < 2:
        raise TypeError("process_inventory_adjustment must return (success, message)")
...
def complete_batch(batch_id):
    ...
    try:
        ...
        if success:
            ...
            flash(message, "error")
            return redirect(
                url_for("batches.view_batch_in_progress", batch_identifier=batch_id)
            )
...
def fail_batch(batch_id):
    ...
    try:
        ...
        reason = (
            request.form.get("reason") or request.json.get("reason")
            if request.is_json
            else None
        )
...
def _complete_batch_internal(batch_id, form_data):
    ...
    try:
            ...
            if product_id and variant_id:
                ...
                for sku in existing_skus:
                    is_valid, error_msg, inv_qty, fifo_total = (
                        validate_inventory_fifo_sync(sku.inventory_item_id, "product")
                    )
                    if not is_valid:
                        return (
                            False,
                            f"Cannot complete batch - inventory sync error for existing SKU {sku.sku_code}: {error_msg}",
                        )
        ...
        if is_perishable:
            ...
            if exp_date_str:
                expiration_date = datetime.strptime(exp_date_str, "%Y-%m-%d").replace(
                    tzinfo=timezone.utc
                )
        ...
        if output_type == "untracked":
            ...
            logger.info(
                f"🔧 BATCH COMPLETION: Creating intermediate ingredient for batch_type='{batch.batch_type}', output_type='{output_type}'"
            )
            _create_intermediate_ingredient(
                batch, final_quantity, output_unit, expiration_date
            )
        else:
            # Handle product creation (batch_type='product' or default)
            logger.info(
                f"🔧 BATCH COMPLETION: Creating product output for batch_type='{batch.batch_type}', output_type='{output_type}'"
            )
            product_id = form_data.get("product_id")
            variant_id = form_data.get("variant_id")
            ...
            output_metrics = _create_product_output(
                batch,
                product_id,
                variant_id,
                final_quantity,
        ...
        try:
            ...
            if stats and batch.final_quantity and output_unit:
                ...
                if isinstance(locals().get("output_metrics"), dict):
                    total_container_volume = output_metrics.get(
                        "total_container_volume"
                    )

                if total_container_volume and total_container_volume > 0:
                    actual_in_containers = min(
                        float(batch.final_quantity), float(total_container_volume)
                    )
                    stats.actual_fill_efficiency = (
                        actual_in_containers / float(total_container_volume)
                    ) * 100.0
                else:
...
def _create_intermediate_ingredient(
    batch, final_quantity, output_unit, expiration_date
):
    ...
    try:
        ...
        logger.info(
            f"🔧 INTERMEDIATE INGREDIENT: Adding {final_quantity} {output_unit} to inventory item {inventory_item.id}"
        )
        ...
        if not success:
            raise ValueError(
                error_message
                or "Failed to add intermediate ingredient inventory via canonical service"
            )

        logger.info(
            f"Created intermediate ingredient via canonical service: {ingredient_name}, quantity: {final_quantity} {output_unit}"
        )

    except Exception as e:
        logger.error(
            f"Error creating intermediate ingredient via canonical service: {str(e)}"
        )
        raise
...
def _create_product_output(
    ...
    try:
        ...
        for extra in batch.extra_ingredients:
            total_ingredient_cost += (extra.quantity_used or 0) * (
                extra.cost_per_unit or 0
            )

        # Calculate ingredient unit cost (cost per unit of final product volume)
        ingredient_unit_cost = (
            total_ingredient_cost / final_quantity if final_quantity > 0 else 0
        )

        logger.info(
            f"Batch {batch.label_code}: Total ingredient cost ${total_ingredient_cost:.2f}, Unit cost ${ingredient_unit_cost:.2f} per {output_unit}"
        )

        # Process container allocations with cost calculation
        container_skus, container_lots = _process_container_allocations(
            batch, product, variant, form_data, expiration_date, ingredient_unit_cost
        )
        ...
        if bulk_quantity > 0 and not getattr(batch, "is_portioned", False):
            ...
            bulk_sku, bulk_lot = _create_bulk_sku(
                product,
                variant,
                bulk_quantity,
                bulk_unit,
            ...
            if bulk_lot:
                logger.info(
                    f"Bulk quantity tied to lot {bulk_lot.display_code} (ID {bulk_lot.id})"
                )
                created_lots.append(bulk_lot)

        # For portioned batches, create portion-based SKU if final_portions provided
        if (
            getattr(batch, "is_portioned", False)
            and final_portions
            and final_portions > 0
        ):
            try:
                # Build portion-based size label
                size_label = _derive_size_label_from_portions(
                    batch, final_quantity, output_unit, final_portions
                )
                ...
                if not sku:
                    ...
                    try:
                        ...
                        template = (
                            ...
                            if category and category.sku_name_template
                            else None
                        ) or "{variant} {product} ({size_label})"
                        naming_context = {
                            "yield_value": final_quantity,
                            "yield_unit": output_unit,
                            "portion_name": getattr(batch, "portion_name", None)
                            or "Unit",
                ...
                if portion_lot:
                    logger.info(
                        f"Portion SKU {sku.sku_code} tied to lot {portion_lot.display_code} (ID {portion_lot.id})"
                    )
        ...
        logger.info(
            f"Created product output for batch {batch.label_code}: {len(container_skus)} container SKUs, {bulk_quantity if not getattr(batch, 'is_portioned', False) else 0} {bulk_unit if (bulk_quantity > 0 and not getattr(batch, 'is_portioned', False)) else ''} bulk"
        )
        return {
            "total_container_volume": total_container_volume,
            "created_lots": [lot for lot in created_lots if lot],
        }
...
def _derive_size_label_from_portions(
    batch, final_bulk_quantity, bulk_unit, final_portions
):
...
def _process_container_allocations(
    batch, product, variant, form_data, expiration_date, ingredient_unit_cost
):
    ...
    container_usage = (
        {}
    )  # container_id -> {'used': total_used, 'passed': passed_to_product}
    ...
    for container in batch.containers:
        ...
        if container_id not in container_usage:
            container_usage[container_id] = {
                "used": 0,
                "passed": 0,
                "cost_each": container.cost_each or 0,
            }
    ...
    for extra_container in batch.extra_containers:
        ...
        if container_id not in container_usage:
            container_usage[container_id] = {
                "used": 0,
                "passed": 0,
                "cost_each": extra_container.cost_each or 0,
            }
    ...
    container_final_keys = [
        k for k in form_data.keys() if k.startswith("container_final_")
    ]
        ...
        if final_quantity > 0:
            try:
                ...
                if not container_item:
                    logger.error(
                        f"Container with ID {container_id} not found for organization {(getattr(current_user, 'organization_id', None) or batch.organization_id)}"
                    )
                    continue
                ...
                adjusted_container_cost_per_unit = (
                    total_container_cost / final_quantity
                    if final_quantity > 0
                    else usage_info["cost_each"]
                )

                # Debug logging
                logger.info(
                    f"Processing container: {container_item.name} (ID: {container_item.id}), {final_quantity} containers"
                )
                logger.info(
                    f"Container cost calculation: {usage_info['used']} used × ${usage_info['cost_each']} = ${total_container_cost}, divided by {final_quantity} passed = ${adjusted_container_cost_per_unit:.2f} per container"
                )
                ...
                container_skus.append(
                    {
                        "sku": container_sku,
                        "quantity": final_quantity,  # Number of containers
                        "container_capacity": container_item.capacity
                        or 1,  # Volume per container
                    }
                )
                ...
                logger.info(
                    f"Created container SKU for {final_quantity} x {container_item.name} containers"
                )
                ...
                logger.error(
                    f"Container processing traceback: {traceback.format_exc()}"
                )
                continue
...
def _create_container_sku(
    ...
    expiration_date,
    ingredient_unit_cost,
    adjusted_container_cost,
):
    """Create or get existing container SKU using ProductService"""
    try:
        logger.info(
            f"Creating container SKU with container: {container_item.name}, quantity: {quantity}"
        )
        ...
        if container_item.capacity and container_item.capacity_unit:
            ...
            cap_str = (
                f"{container_item.capacity} {container_item.capacity_unit}".strip()
            )
            try:
        ...
        total_cost_per_container = (
            ingredient_cost_per_container + adjusted_container_cost
        )

        logger.info(
            f"Container cost breakdown: ${ingredient_cost_per_container:.2f} ingredients + ${adjusted_container_cost:.2f} container = ${total_cost_per_container:.2f} per container"
        )
        ...
        if not success:
            raise ValueError(
                error_message or f"Failed to add container inventory for {size_label}"
            )
        ...
        if new_lot:
            logger.info(
                f"Linked container SKU {product_sku.sku_code} to lot {new_lot.display_code} (ID {new_lot.id})"
            )
...
def _create_bulk_sku(
    product, variant, quantity, unit, expiration_date, batch, ingredient_unit_cost
):
    ...
    try:
        ...
        if not success:
            raise ValueError(
                error_message or f"Failed to add bulk inventory for {quantity} {unit}"
            )
        ...
        if bulk_lot:
            logger.info(
                f"Linked bulk SKU {bulk_sku.sku_code} to lot {bulk_lot.display_code} (ID {bulk_lot.id})"
            )
