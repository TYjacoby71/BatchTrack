...
def debug_inventory(id):
    """Debug endpoint to diagnose FIFO sync issues."""
    item = InventoryItem.query.get_or_404(id)
    if item.organization_id != current_user.organization_id:
    ...
    try:
        ...
        # Get FIFO validation results
        is_valid, error_msg, inv_qty, fifo_total = validate_inventory_fifo_sync(id, item.type)
        
        # Get all FIFO entries
        fifo_entries = InventoryHistory.query.filter_by(inventory_item_id=id).filter(
        ...
        debug_info = {
            ...
            'fifo_entries_count': len(fifo_entries),
            'fifo_entries': [
                {
                    'id': entry.id,
                    ...
                    'note': getattr(entry, 'note', getattr(entry, 'notes', None))
        ...
        logger.error(f"Debug endpoint error for item {id}: {e}", exc_info=True)
