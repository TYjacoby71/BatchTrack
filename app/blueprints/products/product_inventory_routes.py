...
from app.services.product_inventory_route_service import ProductInventoryRouteService
...
product_inventory_bp = Blueprint(
    "product_inventory", __name__, url_prefix="/products/inventory"
)
...
def adjust_sku_inventory(inventory_item_id):
    ...
    logger.info("=== PRODUCT INVENTORY ADJUSTMENT START ===")
    logger.info(f"Inventory Item ID: {inventory_item_id}")
    logger.info(
        f"User: {current_user.id} ({current_user.username if hasattr(current_user, 'username') else 'unknown'})"
    )
    ...
    sku = ProductInventoryRouteService.find_sku_for_inventory_item(
    ...
    if sku:
        logger.info(
            f"SKU details - inventory_item_id: {sku.inventory_item_id}, Product: {sku.product.name if sku.product else 'No product'}, Variant: {sku.variant.name if sku.variant else 'No variant'}"
        )
        ...
        logger.error(
            f"SKU not found for inventory_item_id: {inventory_item_id}, org: {current_user.organization_id}"
        )

    if not sku:
        logger.error(
            f"SKU not found for ID: {inventory_item_id}, org: {current_user.organization_id}"
        )
        if request.is_json:
    ...
    try:
        ...
        if change_type in [
            ...
            if quantity < 0:
                logger.error(
                    f"Quantity should be positive for deduction type: {change_type}"
                )
                raise ValueError("Quantity should be positive for deduction operations")
        elif quantity < 0 and change_type not in ["recount"]:
            logger.error(
                f"Negative quantity not allowed for change_type: {change_type}"
            )
        ...
        logger.info("Optional fields extracted:")
        ...
        if data.get("shelf_life_days"):
            try:
                ...
                if data.get("expiration_date"):
                    ...
                    custom_expiration_date = datetime.strptime(
        ...
        try:
            ...
            response_message = service_message or (
                "Product inventory adjusted successfully"
                if success
                else "Error adjusting product inventory"
            )

            if request.is_json:
                status_code = 200 if success else 400
                return (
                    jsonify({"success": success, "message": response_message}),
                    status_code,
                )
        ...
        ProductInventoryRouteService.rollback_session()
    ...
    logger.info("=== PRODUCT INVENTORY ADJUSTMENT END ===")
...
def get_sku_fifo_status(sku_id):
    ...
    logger.info(
        f"Found SKU: {sku.inventory_item_id}, Product: {sku.product.name if sku.product else 'Unknown'}"
    )

    today = TimezoneUtils.utc_now().date()
    inventory_item_id = sku.inventory_item_id

    fresh_lots = ProductInventoryRouteService.list_fresh_inventory_lots(
        inventory_item_id=inventory_item_id,
        organization_id=current_user.organization_id,
        as_of_date=today,
    )

    expired_lots = ProductInventoryRouteService.list_expired_inventory_lots(
        inventory_item_id=inventory_item_id,
        organization_id=current_user.organization_id,
        as_of_date=today,
    )
...
def process_sale_webhook():
    ...
    sku = ProductInventoryRouteService.find_active_sku_by_code(
    ...
    try:
        ...
        if success:
            ProductInventoryRouteService.commit_session()
            return jsonify(
                {
                    "success": True,
...
def create_manual_reservation():
    ...
    if not has_permission("inventory.reserve"):
        if request.is_json:
            return (
                jsonify({"error": "Insufficient permissions to create reservations"}),
                403,
            )
...
def add_inventory_from_batch():
    ...
    try:
        ...
        if product_id:
            base_sku = ProductInventoryRouteService.find_sku_by_id_for_org(
                sku_id=product_id,
                organization_id=current_user.organization_id,
            )
            if not base_sku:
