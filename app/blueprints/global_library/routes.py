    ...
    try:
        ...
        if last_ts:
            ...
            if datetime.now(timezone.utc) - last_dt > timedelta(
                hours=PUBLIC_LIBRARY_WINDOW_HOURS
            ):
                return limit
    except Exception:
        return limit
...
def global_library():
    ...
    normalized_scope = (
        scope_param or GLOBAL_LIBRARY_DEFAULT_SCOPE
    ).lower() or GLOBAL_LIBRARY_DEFAULT_SCOPE
    ...
    if not current_user.is_authenticated:
        ...
        search_remaining = _remaining_public_counter(
            "global_library_search_views", PUBLIC_LIBRARY_SEARCH_LIMIT
        )
        if search_query:
    ...
    if active_scope:
        description_bits.insert(
            0,
            f"{GLOBAL_SCOPE_LABELS.get(active_scope, active_scope.title())} in the BatchTrack library.",
        )
    ...
    rendered = render_template(
        ...
        developer_link_base=(
            url_for("developer.global_item_detail", item_id=0)[:-1]
            if can_manage
            else None
        ),
    ...
    if not bypass_cache:
        cache.set(
            cache_key,
            rendered,
            timeout=current_app.config.get("GLOBAL_LIBRARY_CACHE_TTL", 300),
        )
    return rendered
...
def global_item_detail(item_id: int, slug: Optional[str] = None):
    ...
    description = (
        ...
        or f"{gi.name} specs, density, and usage guidance from the BatchTrack global library."
    )
    page_title = (
        metadata.get("meta_title")
        or f"{gi.name} — {gi.item_type.capitalize()} Reference"
    )
...
def global_library_item_stats(item_id: int):
    ...
    try:
        ...
        item_payload = {
            ...
            "variation": (
                {
                    ...
                    "physical_form_name": (
                        gi.variation.physical_form.name
                        if getattr(gi.variation, "physical_form", None)
                        else None
                    ),
                    "physical_form_slug": (
                        gi.variation.physical_form.slug
                        if getattr(gi.variation, "physical_form", None)
                        else None
                    ),
                }
                if getattr(gi, "variation", None)
                else None
            ),
            # Backwards-compatible physical form fields (derived from variation when present)
            "physical_form": (
                gi.physical_form.name if getattr(gi, "physical_form", None) else None
            ),
            "physical_form_slug": (
                gi.physical_form.slug if getattr(gi, "physical_form", None) else None
            ),
            ...
            "brewing_diastatic_power_lintner": getattr(
                gi, "brewing_diastatic_power_lintner", None
            ),
            # Certifications
        ...
        logger = logging.getLogger(__name__)
        logger.error(
            f"Error getting stats for global item {item_id}: {str(e)}", exc_info=True
        )
        from flask import jsonify
