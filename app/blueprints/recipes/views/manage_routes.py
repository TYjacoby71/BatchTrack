...
from app.services.recipe_manage_view_service import RecipeManageViewService
...
def list_recipes():
    ...
    pagination = RecipeManageViewService.paginate_master_recipes(
    ...
    group_variations = RecipeManageViewService.list_group_variations_for_masters(
        recipes=recipes,
        organization_id=getattr(current_user, "organization_id", None),
    )
    ...
    try:
        ...
        logger.warning(
            "Suppressed exception fallback at app/blueprints/recipes/views/manage_routes.py:202",
            exc_info=True,
        )
        pass
    return rendered
...
def view_recipe(recipe_id):
    try:
        ...
        variation_count = RecipeManageViewService.count_active_variations_for_recipe(
            recipe=recipe
        )
        ...
        recipe_notes = RecipeManageViewService.list_recent_recipe_notes(
            recipe_id=recipe.id,
            limit=25,
        )
        ...
        if recipe.recipe_group_id:
            group_versions = RecipeManageViewService.list_group_versions(
        ...
        if recipe.is_master and recipe.test_sequence is None and recipe.recipe_group_id:
            origin_parent_recipe = RecipeManageViewService.find_previous_master_version(
                recipe_group_id=recipe.recipe_group_id,
        ...
        logger.warning(
            "Suppressed exception fallback at app/blueprints/recipes/views/manage_routes.py:347",
            exc_info=True,
        )
...
def add_recipe_note(recipe_id):
    recipe = RecipeManageViewService.get_recipe_by_id(recipe_id=recipe_id)
    if not recipe:
        flash("Recipe not found.", "error")
        return redirect(url_for("recipes.list_recipes"))
    ...
    try:
        RecipeManageViewService.add_note_entry(
            recipe=recipe,
            note_text=note_text,
        ...
        logger.warning(
            "Suppressed exception fallback at app/blueprints/recipes/views/manage_routes.py:385",
            exc_info=True,
        )
        RecipeManageViewService.rollback_session()
...
def make_parent_recipe(recipe_id):
    try:
        status, recipe, original_parent = (
            RecipeManageViewService.convert_variation_to_parent(
                recipe_id=recipe_id,
        ...
        logger.warning(
            "Suppressed exception fallback at app/blueprints/recipes/views/manage_routes.py:464",
            exc_info=True,
        )
        RecipeManageViewService.rollback_session()
...
def lock_recipe(recipe_id):
    recipe = RecipeManageViewService.get_scoped_recipe_or_404(recipe_id=recipe_id)
    recipe.is_locked = True
    RecipeManageViewService.commit_session()
...
def publish_test_version(recipe_id):
    try:
        ...
        logger.warning(
            "Suppressed exception fallback at app/blueprints/recipes/views/manage_routes.py:528",
            exc_info=True,
        )
        RecipeManageViewService.rollback_session()
...
def set_current_version_route(recipe_id):
    try:
        ...
        logger.warning(
            "Suppressed exception fallback at app/blueprints/recipes/views/manage_routes.py:550",
            exc_info=True,
        )
        RecipeManageViewService.rollback_session()
...
def promote_variation_to_master(recipe_id):
    try:
        ...
        logger.warning(
            "Suppressed exception fallback at app/blueprints/recipes/views/manage_routes.py:572",
            exc_info=True,
        )
        RecipeManageViewService.rollback_session()
...
def promote_variation_to_new_group(recipe_id):
    try:
        ...
        logger.warning(
            "Suppressed exception fallback at app/blueprints/recipes/views/manage_routes.py:605",
            exc_info=True,
        )
        RecipeManageViewService.rollback_session()
