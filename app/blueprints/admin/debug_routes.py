...
def validate_all_fifo_sync():
    ...
    items = InventoryItem.query.filter_by(organization_id=current_user.organization_id).all()
    ...
    if sync_issues:
        logger.warning(f"FIFO sync issues found for {len(sync_issues)} items in org {current_user.organization_id}")
