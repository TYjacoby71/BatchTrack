...
class UnifiedInventoryHistory(ScopedModelMixin, db.Model):
    ...
    affected_lot_id = db.Column(
        db.Integer, db.ForeignKey("inventory_lot.id"), nullable=True
    )
    ...
    fifo_reference_id = db.Column(
        db.Integer, db.ForeignKey("unified_inventory_history.id"), nullable=True
    )
    ...
    inventory_item = db.relationship(
        "InventoryItem", foreign_keys=[inventory_item_id], backref="unified_history"
    )
    affected_lot = db.relationship(
        "InventoryLot", foreign_keys=[affected_lot_id], backref="events"
    )
