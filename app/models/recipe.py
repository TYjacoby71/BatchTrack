...
class RecipeGroup(ScopedModelMixin, db.Model):
    ...
    __table_args__ = (
        ...
        db.UniqueConstraint(
            "organization_id", "prefix", name="uq_recipe_group_org_prefix"
        ),
...
class Recipe(ScopedModelMixin, db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    recipe_group_id = db.Column(
        db.Integer, db.ForeignKey("recipe_group.id"), nullable=True
    )
    is_master = db.Column(
        db.Boolean, default=True, nullable=False, server_default=sa.text("true")
    )
    ...
    is_current = db.Column(
        db.Boolean, default=False, nullable=False, server_default=sa.text("false")
    )
    ...
    status = db.Column(
        db.String(16), default="published", nullable=False, server_default="published"
    )
    sharing_scope = db.Column(
        db.String(16), nullable=False, default="private", server_default="private"
    )
    ...
    recipe_consumables = db.relationship(
        "RecipeConsumable", backref="recipe", cascade="all, delete-orphan"
    )
    moderation_events = db.relationship(
        "RecipeModerationEvent", backref="recipe", cascade="all, delete-orphan"
    )
    ...
    marketplace_status = db.Column(
        db.String(32), nullable=False, default="draft", server_default="draft"
    )
    marketplace_notes = db.Column(db.Text, nullable=True)
    marketplace_violation_count = db.Column(
        db.Integer, nullable=False, default=0, server_default="0"
    )
    marketplace_blocked = db.Column(
        db.Boolean, nullable=False, default=False, server_default=sa.text("false")
    )
    ...
    org_origin_recipe_id = db.Column(
        db.Integer, db.ForeignKey("recipe.id"), nullable=True
    )
    org_origin_type = db.Column(
        db.String(32), nullable=False, default="authored", server_default="authored"
    )
    org_origin_source_org_id = db.Column(
        db.Integer, db.ForeignKey("organization.id"), nullable=True
    )
    org_origin_source_recipe_id = db.Column(
        db.Integer, db.ForeignKey("recipe.id"), nullable=True
    )
    org_origin_purchased = db.Column(
        db.Boolean, nullable=False, default=False, server_default=sa.text("false")
    )
    is_sellable = db.Column(
        db.Boolean, nullable=False, default=True, server_default=sa.text("true")
    ...
    organization = db.relationship(
        "Organization", foreign_keys="Recipe.organization_id"
    )
    ...
    origin_organization = db.relationship(
        "Organization", foreign_keys=[origin_organization_id]
    )
    ...
    org_origin_source_org = db.relationship(
        "Organization", foreign_keys=[org_origin_source_org_id]
    )
    ...
    soap_superfat = db.Column(
        sa.Numeric(),
        _pg_computed("((category_data ->> 'soap_superfat'))::numeric"),
        nullable=True,
    )
    soap_water_pct = db.Column(
        sa.Numeric(),
        _pg_computed("((category_data ->> 'soap_water_pct'))::numeric"),
        nullable=True,
    )
    soap_lye_type = db.Column(
        sa.Text(), _pg_computed("(category_data ->> 'soap_lye_type')"), nullable=True
    )

    candle_fragrance_pct = db.Column(
        sa.Numeric(),
        _pg_computed("((category_data ->> 'candle_fragrance_pct'))::numeric"),
        nullable=True,
    )
    candle_vessel_ml = db.Column(
        sa.Numeric(),
        _pg_computed("((category_data ->> 'candle_vessel_ml'))::numeric"),
        nullable=True,
    )
    vessel_fill_pct = db.Column(
        sa.Numeric(),
        _pg_computed("((category_data ->> 'vessel_fill_pct'))::numeric"),
        nullable=True,
    )

    baker_base_flour_g = db.Column(
        sa.Numeric(),
        _pg_computed("((category_data ->> 'baker_base_flour_g'))::numeric"),
        nullable=True,
    )
    baker_water_pct = db.Column(
        sa.Numeric(),
        _pg_computed("((category_data ->> 'baker_water_pct'))::numeric"),
        nullable=True,
    )
    baker_salt_pct = db.Column(
        sa.Numeric(),
        _pg_computed("((category_data ->> 'baker_salt_pct'))::numeric"),
        nullable=True,
    )
    baker_yeast_pct = db.Column(
        sa.Numeric(),
        _pg_computed("((category_data ->> 'baker_yeast_pct'))::numeric"),
        nullable=True,
    )

    cosm_emulsifier_pct = db.Column(
        sa.Numeric(),
        _pg_computed("((category_data ->> 'cosm_emulsifier_pct'))::numeric"),
        nullable=True,
    )
    cosm_preservative_pct = db.Column(
        sa.Numeric(),
        _pg_computed("((category_data ->> 'cosm_preservative_pct'))::numeric"),
        nullable=True,
    )
...
class RecipeConsumable(ScopedModelMixin, db.Model):
    ...
    inventory_item = db.relationship(
        "InventoryItem", backref="recipe_consumable_usages"
    )
...
class RecipeLineage(ScopedModelMixin, db.Model):
    ...
    recipe = db.relationship(
        "Recipe", foreign_keys=[recipe_id], backref="lineage_events"
    )
    source_recipe = db.relationship(
        "Recipe",
        foreign_keys=[source_recipe_id],
        backref="lineage_source_events",
        viewonly=True,
    )
