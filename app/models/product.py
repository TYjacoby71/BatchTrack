...
class ProductSKU(ScopedModelMixin, db.Model):
    ...
    def reserved_quantity(self):
        """Get currently reserved quantity from FIFO allocation entries"""
        ...
        # Sum up all reservation allocation entries with remaining quantity
        reserved_allocations = ProductSKUHistory.query.filter(
            and_(
                ProductSKUHistory.inventory_item_id == self.inventory_item_id,
                ProductSKUHistory.change_type == 'reserved_allocation',
                ProductSKUHistory.remaining_quantity > 0,
                ProductSKUHistory.organization_id == current_user.organization_id if current_user and current_user.is_authenticated else self.inventory_item.organization_id
            )
        ).all()

        return sum(entry.remaining_quantity for entry in reserved_allocations)
    ...
    def available_quantity_for_sale(self):
        """Get quantity available for sale (total inventory minus reserved)"""
        if not self.inventory_item:
            return 0

        # item.quantity now represents only available stock (FIFO-eligible)
        # Reserved stock is tracked separately and doesn't count against item.quantity
