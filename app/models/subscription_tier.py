...
subscription_tier_permission = db.Table(
    "subscription_tier_permission",
    db.Column(
        "tier_id", db.Integer, db.ForeignKey("subscription_tier.id"), primary_key=True
    ),
    db.Column(
...
class SubscriptionTier(db.Model):
    ...
    tier_type = db.Column(
        db.String(32), nullable=False, default="monthly"
    ...
    __table_args__ = (
        db.UniqueConstraint(
            "stripe_lookup_key", name="uq_subscription_tier_stripe_lookup_key"
        ),
        db.UniqueConstraint(
            "stripe_storage_lookup_key",
            name="uq_subscription_tier_stripe_storage_lookup_key",
        ),
        db.UniqueConstraint(
            "whop_product_key", name="uq_subscription_tier_whop_product_key"
        ),
    )
    ...
    def find_by_identifier(cls, identifier):
        ...
        if normalized == "exempt":
            tier = (
                cls.query.filter_by(billing_provider="exempt")
                .order_by(cls.id.asc())
                .first()
            )
            if tier:
