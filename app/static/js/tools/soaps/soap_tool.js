(function(){
  const TOOL_UNIT_OPTIONS_HTML = (window.soapToolConfig && window.soapToolConfig.unitOptionsHtml) || '';
  ...
  const STAGE_CONFIGS = [
    ...
    { id: 7, tabId: 'soapStage7Tab', paneId: 'soapStage7Pane', required: false },
  ];
  ...
  window.SoapCalcService = SoapCalcService;
  ...
  if (stageTabList) {
    stageTabList.addEventListener('shown.bs.tab', scheduleStageHeightSync);
  }
  ...
  if (soapRoot) {
    ...
    soapRoot.addEventListener('input', function(e){
      if (e.target.matches('input, select, textarea')) {
        ...
        updateStageStatuses();
        flashStage(e.target.closest('.soap-stage-card'));
      }
    });
