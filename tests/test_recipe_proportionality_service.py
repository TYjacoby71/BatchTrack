...
def _ensure_unit(
    name: str, unit_type: str, conversion_factor: float, base_unit: str | None = None
):
...
def test_proportionality_matches_scaled_payloads():
    ...
    assert RecipeProportionalityService.are_recipes_proportionally_identical(
        payload_a, payload_b
    )
...
def test_proportionality_detects_ratio_changes():
    ...
    assert not RecipeProportionalityService.are_recipes_proportionally_identical(
        base, changed
    )
