...
def test_auto_fill_containers_uses_available_capacity(app, db_session):
    from app.models import InventoryItem, Organization, ProductCategory, Recipe, Unit
    ...
    with app.app_context():
        ...
        strategy, container_options = analyze_container_options(
            recipe=recipe, scale=1.0, organization_id=org.id, api_format=False
        )

        assert (
            strategy is not None
        ), "Strategy should be generated for matching container units"
        ...
        assert (
            selection[0]["containers_needed"] == 3
        ), "12 fl oz yield should use three 4 fl oz containers"
