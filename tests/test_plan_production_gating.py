...
from app.models.recipe import Recipe, RecipeConsumable, RecipeIngredient
...
def _build_recipe_with_missing_inventory(
    *,
    ...
    if not perm:
        ...
        db.session.add(perm)
        db.session.flush()
    output_tracking_perm = Permission.query.filter_by(
    ...
    if not output_tracking_perm:
        ...
        db.session.flush()
    quantity_tracking_perm = Permission.query.filter_by(
    ...
    tier = SubscriptionTier(name="Gating Tier", billing_provider="exempt", user_limit=5)
    db.session.add(tier)
    db.session.flush()
    ...
    ingredient = InventoryItem(
        name="Test Oil", unit="g", quantity=0, organization_id=org.id, type="ingredient"
    )
...
def test_api_start_batch_requires_override_before_force(app, monkeypatch):
    ...
    with app.app_context():
        user_id, recipe_id, ingredient_id, consumable_id = (
            _build_recipe_with_missing_inventory(
        ...
        monkeypatch.setattr(
            BatchOperationsService, "start_batch", classmethod(fake_start_batch)
        )
        ...
        with app.test_request_context(
            "/batches/api/start-batch", method="POST", json=payload
        ):
            ...
            assert data[
                "stock_issues"
            ], "Expected stock issues to be returned when gating triggers"
...
def test_api_start_batch_forces_untracked_when_output_tracking_tier_permission_missing(
    app, monkeypatch
):
    app.config["SKIP_PERMISSIONS"] = True
    with app.app_context():
...
def test_api_start_batch_allows_untracked_inventory_items_without_force(
    app, monkeypatch
):
    app.config["SKIP_PERMISSIONS"] = True
    with app.app_context():
