...
def test_expiration_service_uses_canonical_adjustment(app, db_session):
    ...
    fifo_entry = InventoryHistory(
        inventory_item_id=item.id, remaining_quantity=50, unit="count"
    )
    ...
    with patch(
        ...
        ExpirationService.mark_as_expired("fifo", fifo_entry.id, 50, "Test expiration")
...
class TestExpirationCanonicalService:
    ...
    def test_mark_fifo_expired_calls_canonical_service(
        ...
        with app.app_context():
            ...
            success, message = ExpirationService.mark_as_expired(
                "fifo", 123, quantity=5.0, notes="Test expiration"
            )
    ...
    def test_mark_product_expired_calls_canonical_service(
        ...
        with app.app_context():
            ...
            success, message = ExpirationService.mark_as_expired(
                "product", 789, quantity=15.0, notes="Product expired"
            )
