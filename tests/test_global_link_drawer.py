...
def test_global_link_suggestions_and_link_flow(app, db_session):
    ...
    if not Unit.query.filter_by(name="g").first():
        db_session.add(
            Unit(name="g", symbol="g", unit_type="weight", conversion_factor=1.0)
        )
    if not Unit.query.filter_by(name="ml").first():
        db_session.add(
            Unit(name="ml", symbol="ml", unit_type="volume", conversion_factor=1.0)
        )
    if not Unit.query.filter_by(name="count").first():
        db_session.add(
            Unit(name="count", symbol="ct", unit_type="count", conversion_factor=1.0)
        )
    db_session.commit()
    ...
    milk_ml = InventoryItem(
        name="milk", type="ingredient", unit="ml", quantity=0.0, organization_id=org.id
    )
    milk_g = InventoryItem(
        name="MILK", type="ingredient", unit="g", quantity=0.0, organization_id=org.id
    )
    milk_count = InventoryItem(
        name="milk (units)",
        type="ingredient",
        unit="count",
        quantity=0.0,
    ...
    with app.test_client() as client:
        ...
        res2 = client.get(
            f"/api/drawers/global-link/modal?global_item_id={global_item_id}"
        )
        ...
        payload = {"global_item_id": global_item_id, "item_ids": [milk_ml_id]}
        res3 = client.post(
            "/api/drawers/global-link/confirm",
