...
class TestSignupTierCharacterization:
    ...
    def test_find_by_identifier_handles_multiple_keys(self, app):
        ...
        with app.app_context():
            ...
            assert SubscriptionTier.find_by_identifier(tier.stripe_lookup_key).id == tier.id

    def test_signup_service_creates_pending_signup_record_with_placeholder_email(self, app):
        """SignupService should normalize blank emails before redirecting to Stripe."""
        with app.app_context():
            tier = SubscriptionTier(name=_unique('Solo'), billing_provider='stripe', stripe_lookup_key=_unique('price_solo'))
            db.session.add(tier)
            db.session.commit()
            ...
            pending = SignupService.create_pending_signup_record(
                ...
                oauth_user_info={'oauth_provider': 'google', 'oauth_provider_id': 'abc123'},
    ...
    def test_organization_user_limit_respects_tier_configuration(self, app):
        ...
        with app.app_context():
            ...
            user = User(username=_unique('owner'), email=_unique('owner') + '@example.com', organization_id=org.id, is_active=True)
            db.session.add(user)
            db.session.commit()
