...
| Controller vs business logic | 8.6/10 | 16 blueprint modules still contain direct query/session logic after extracting SKU route lookup/history/reservation/merge query and session touchpoints into service boundaries on top of prior batches. |
| Data access ownership | 8.6/10 | Persistence logic remains mixed into route layers, but this pass removed additional route-local query/session touchpoints from SKU route handlers. |
...
- Total blueprint files with direct query/session access: **16**.
...
- Data access findings overlap controller/business findings: persistence logic is still route-adjacent in 16 files.
