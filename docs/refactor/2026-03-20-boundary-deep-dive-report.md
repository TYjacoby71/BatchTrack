...
| Controller vs business logic | 6.2/10 | 50 blueprint modules still contain direct query/session logic after extracting tag-manager and recipe-form payload persistence logic into services. |
| Data access ownership | 6.2/10 | Persistence logic remains mixed into route layers, but additional route-local query/mutation paths were removed this pass. |
...
- Total blueprint files with direct query/session access: **50**.
...
- Data access findings overlap controller/business findings: persistence logic is still route-adjacent in 50 files.
