...
| Controller vs business logic | 8.3/10 | 19 blueprint modules still contain direct query/session logic after extracting recipe-create anti-plagiarism/prefill/import lookup paths into service boundaries on top of prior batches. |
| Data access ownership | 8.3/10 | Persistence logic remains mixed into route layers, but this pass removed additional route-local query/session touchpoints from recipe-create route handlers. |
...
- Total blueprint files with direct query/session access: **19**.
...
- Data access findings overlap controller/business findings: persistence logic is still route-adjacent in 19 files.
