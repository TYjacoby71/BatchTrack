...
| Controller vs business logic | 6.1/10 | 52 blueprint modules still contain direct query/session logic after extracting integration-admin and billing webhook persistence logic. |
| Data access ownership | 6.1/10 | Persistence logic remains mixed into route layers, but additional developer/billing route-local DB paths were removed this pass. |
...
- Total blueprint files with direct query/session access: **52**.
...
- Data access findings overlap controller/business findings: persistence logic is still route-adjacent in 52 files.
