...
| Redis pooling | `REDIS_POOL_MAX_CONNECTIONS=10`, `REDIS_POOL_TIMEOUT=5`, `REDIS_SOCKET_TIMEOUT=5`, `REDIS_CONNECT_TIMEOUT=5` | Set per-worker pool caps | Prevents run-away Redis client creation under bursty load. |
...
- Redis connections are pooled and shared across sessions, caching, and rate limiting; set `REDIS_POOL_MAX_CONNECTIONS` to cap per-worker usage (default 10).
