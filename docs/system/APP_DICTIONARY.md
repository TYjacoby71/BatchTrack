...
- **/auth/login (inactive org lockout)** → Login denies customer access when organization is hard-locked (explicit org inactive, or non-exempt tier in `suspended`/`canceled` state) and instructs users to contact support (see `app/blueprints/auth/login_routes.py` and `app/services/billing_access_policy_service.py`)
...
- **/api/fifo-details/<inventory_id>** → FIFO detail payload for inventory and batch-usage modals (see `app/blueprints/api/routes.py` and `app/services/fifo_api_service.py`)
- **/batches/api/batch-inventory-summary/<batch_id>** → Batch FIFO summary payload (see `app/blueprints/batches/routes.py` and `app/services/fifo_api_service.py`)
...
- **Unit quick-create client modal** → Browser-rendered modal component that creates units via `POST /api/units` without a drawer route roundtrip (see `app/static/js/drawers/unit_quick_create_modal.js` and `app/templates/pages/recipes/recipe_form.html`)
...
- **Reservation API routes** → Reservation create/release/confirm/details/cleanup endpoints for SKU stock-hold lifecycle handling and POS order flows (see `app/blueprints/products/reservation_routes.py`)
- **Unit API routes** → Authenticated unit list/search/create and conversion endpoints for inventory/recipe tooling (see `app/blueprints/api/routes.py`, `app/services/unit_catalog_service.py`, and `app/blueprints/recipes/views/ajax_routes.py`)
...
- **AuthBillingOrchestrator** → Auth-facing billing facade used by middleware/login/permissions to share one policy interpretation and standing checks (see `app/services/billing/orchestrators/auth_billing_orchestrator.py`)
- **Billing Orchestrators Package** → Workflow facades for public signup, auth billing, settings billing context, and checkout-account provisioning (see `app/services/billing/orchestrators/`)
...
- **Global Client Notification Helper** → Shared browser notification API (`window.showAlert` / `window.showNotification`) that normalizes severity, supports legacy argument order, and renders dismissible Bootstrap alerts instead of native browser popups (see `app/static/js/main.js`)
- **Global Client Toast Helper** → Shared browser toast API (`window.showToast`) that renders Bootstrap toasts in a single global toast container for ephemeral feedback like copy/success acknowledgements (see `app/static/js/main.js`)
- **Global Client Confirmation Modal Helper** → Shared promise-based confirmation API (`window.showConfirmDialog`) that renders a reusable Bootstrap modal for destructive/decision flows and only falls back to native confirm when Bootstrap modal runtime is unavailable (see `app/static/js/main.js`)
- **Global Client Prompt Modal Helper** → Shared promise-based input API (`window.showPromptDialog`) that provides modal-backed prompt flows for naming/confirmation text inputs and preserves native prompt as a guarded fallback only when Bootstrap modal runtime is unavailable (see `app/static/js/main.js`)
- **Declarative Confirm Attributes (`data-confirm-star`)** → HTML-level confirmation convention where forms/anchors provide confirmation metadata and shared runtime handlers route blocking decisions through global confirmation modals (see `app/static/js/main.js` and templates under `app/templates/`)
- **Legacy Alert Compatibility Shim (Retired)** → Prior runtime shim that routed legacy `alert(...)` calls through `window.showAlert`; retired after source alert callsites were migrated to direct shared-helper usage (see `app/static/js/main.js` and `docs/system/NOTIFICATION_UX_STANDARD.md`)
- **Pop-up Guard Helper (`openWindowOrNotify`)** → Shared helper for `window.open` flows that emits a warning notification when pop-ups are blocked instead of silently failing (see `app/static/js/main.js`)
- **Notification Primitive Guard Script** → Diff-based validation utility that blocks newly added raw native dialog calls (`alert`/`confirm`/`prompt`) outside approved fallback contexts during finalization workflow checks (see `scripts/validate_notification_primitives.py` and `.github/workflows/documentation-guard.yml`)
- **System Status Response Surface** → Request-level status messaging pattern for `403`/`404`/`429`/`5xx` outcomes where HTTP status and payload/page are canonical, with optional client mapping into inline alerts (see `app/resilience.py`, `app/middleware/registry.py`, and `app/blueprints/api/routes.py`)
- **Global Error Status Pages** → Lightweight full-page fallback templates for navigation/server failures (`404`, `500`, maintenance `503`) rendered by resilience handlers when HTML is expected (see `app/templates/errors/404.html`, `app/templates/errors/500.html`, `app/templates/errors/maintenance.html`, and `app/resilience.py`)
- **Quota/Rate-Limit API Notification Payload** → JSON response contract for exceeded usage limits that returns `429` plus machine-readable keys (for example `limit_reached`, refill URLs, or usage windows) for client UX handling (see `app/blueprints/tools/routes.py` and `app/blueprints/api/routes.py`)
- **Notification Standardization Phase 1 (ALRT-001..010)** → First migration batch that removed direct native alert/confirm usage from production-planning stock/batch modules, drawer-protocol fallback errors, batch form actions, expiration actions, inventory infinite-toggle confirmation, settings bulk updates, inventory adjustment validation, and manual-container guidance (see `app/static/js/production_planning/modules/stock-check.js`, `app/static/js/production_planning/modules/batch-management.js`, `app/static/js/core/DrawerProtocol.js`, `app/static/js/batches/batch_form.js`, `app/static/js/expiration_alerts.js`, `app/static/js/inventory/inventory_view.js`, `app/static/js/settings.js`, `app/static/js/inventory/inventory_adjust.js`, and `app/static/js/production_planning/modules/manual-container-mode.js`)
- **Notification Standardization Phase 2 (ALRT-011..030)** → Second migration batch that replaced remaining unit-conversion and organization-dashboard native dialogs with shared modal/toast helpers, removed fallback native alerts in tier/product modules, and removed local flash rendering blocks from login/unit-management templates in favor of layout chrome rendering (see `app/static/js/conversion/unit_converter.js`, `app/static/js/organization/dashboard.js`, `app/static/js/subscription_tiers.js`, `app/static/js/products/product_inventory.js`, `app/templates/pages/auth/login.html`, and `app/templates/conversion/units.html`)
- **Notification Standardization Phase 3 (ALRT-031..040)** → Backend flash hygiene pass that standardized implicit default-category flashes to explicit severity categories across recipe creation/management and recipe-library preview-limit flows to keep global flash rendering deterministic (see `app/blueprints/recipes/views/create_routes.py`, `app/blueprints/recipes/views/manage_routes.py`, and `app/blueprints/recipe_library/routes.py`)
- **Notification Standardization Phase 4 (ALRT-041..050)** → Backend flash-category normalization and validation pass across onboarding, auth, bulk stock, and global library blueprints, ensuring all user-facing redirect/status flashes explicitly declare severity categories for consistent global rendering (see `app/blueprints/onboarding/routes.py`, `app/blueprints/conversion/routes.py`, `app/blueprints/bulk_stock/routes.py`, `app/blueprints/auth/verification_routes.py`, `app/blueprints/auth/whop_routes.py`, `app/blueprints/auth/password_routes.py`, `app/blueprints/auth/login_routes.py`, `app/blueprints/auth/oauth_routes.py`, `app/blueprints/auth/signup_routes.py`, and `app/blueprints/global_library/routes.py`)
- **Notification Standardization Phase 5 (ALRT-051..069)** → Final backend flash-category pass covering remaining inventory, billing, dashboard, developer, and batch blueprints to eliminate implicit/default-category flashes and keep global severity mapping deterministic across all tracked checklist callsites (see `app/blueprints/inventory/routes.py`, `app/blueprints/auth/whop_auth.py`, `app/blueprints/billing/routes.py`, `app/blueprints/dashboard/routes.py`, `app/blueprints/developer/subscription_tiers.py`, `app/blueprints/developer/views/organization_routes.py`, `app/blueprints/developer/views/reference_routes.py`, `app/blueprints/developer/views/affiliate_routes.py`, `app/blueprints/developer/views/product_category_routes.py`, `app/blueprints/batches/start_batch.py`, `app/blueprints/developer/views/user_routes.py`, `app/blueprints/batches/cancel_batch.py`, `app/blueprints/developer/views/global_item_routes.py`, `app/blueprints/batches/finish_batch.py`, `app/blueprints/developer/views/analytics_routes.py`, `app/blueprints/batches/routes.py`, `app/blueprints/developer/views/masquerade_routes.py`, `app/blueprints/developer/decorators.py`, and `app/blueprints/developer/addons.py`)
- **Notification Standardization Phase 6 (ALRT-070..080)** → Extension pass that cataloged and normalized non-flash system-level surfaces including global error handlers/pages, shared layout verification/upgrade overlays, popup-blocked warnings, and API/security/quota payload contracts so these notification channels are tracked with the same checklist rigor (see `app/resilience.py`, `app/templates/errors/404.html`, `app/templates/errors/500.html`, `app/templates/errors/maintenance.html`, `app/templates/components/layout/content_chrome.html`, `app/templates/components/layout/overlay_modals.html`, `app/blueprints/tools/routes.py`, `app/blueprints/api/routes.py`, `app/static/js/tools/soaps/soap_tool_events_exports.js`, and `app/middleware/registry.py`)
- **Notification Standardization Phase 7 (ALRT-081..087)** → Native-dialog normalization pass that introduced shared prompt/declarative confirm infrastructure, migrated high-risk confirm/prompt flows in templates, standardized pop-up blocked handling, enabled lightweight-shell helper loading parity, and added a diff-based primitive guard to prevent regressions (see `app/static/js/main.js`, `app/templates/components/layout/scripts_primary.html`, `app/templates/components/shared/user_management_modal.html`, `app/templates/inventory/bulk_updates.html`, `app/templates/developer/system_roles.html`, `app/templates/pages/recipes/recipe_form.html`, `app/templates/pages/inventory/view.html`, `app/templates/expiration/alerts.html`, `app/templates/admin/reservations.html`, `app/static/js/core/DrawerProtocol.js`, `app/static/js/organization/dashboard.js`, `scripts/validate_notification_primitives.py`, and `.github/workflows/documentation-guard.yml`)
- **Notification Standardization Phase 8 (ALRT-088)** → Legacy-alert retirement pass that migrated remaining source-template native `alert(...)` usage to `window.showAlert(...)` and retired `main.js` alert-shim installation (see `app/templates/`, `app/static/js/main.js`, and `docs/system/NOTIFICATION_UX_STANDARD.md`)
...
- **Soap Tool Dist Bundle Mapping (Hashed)** → Fingerprinted soap tool bundle outputs and manifest mapping used by static asset resolution so updated runtime code ships through hashed frontend assets (see `app/static/dist/js/tools/soaps/`, `app/static/dist/js/tools/soaps/`, `app/static/dist/js/tools/soaps/`, `app/static/dist/js/tools/soaps/`, and `app/static/dist/manifest.json`)
...
- **Soap Static Asset Manifest Outputs** → Hashed soap bundle map and generated runtime bundles emitted by the scoped asset build pipeline for production cache-busting (see `app/static/dist/manifest.json`, `app/static/dist/js/tools/soaps/`, and `app/static/dist/js/tools/soaps/`)
...
- **ENFORCE_EDGE_ORIGIN_AUTH** → Security toggle that enables origin-side validation of an edge-injected shared header before serving non-exempt paths (see `app/config_schema_parts/security.py` and `app/middleware/guards.py`)
- **EDGE_ORIGIN_AUTH_HEADER** → Configured request-header name expected from the edge/WAF when origin-auth enforcement is enabled (see `app/config_schema_parts/security.py` and `app/middleware/guards.py`)
- **EDGE_ORIGIN_AUTH_SECRET** → Shared secret value compared against the configured edge header to validate edge-to-origin request authenticity (see `app/config_schema_parts/security.py` and `app/middleware/guards.py`)
- **EDGE_ORIGIN_AUTH_EXEMPT_PATHS** → Comma-separated exact/prefix path rules excluded from edge-origin auth checks (health and other explicitly exempt endpoints) (see `app/config_schema_parts/security.py` and `app/middleware/guards.py`)
- **X-Edge-Origin-Auth** → Default edge-injected request-header name used by Cloudflare transform rules and middleware validation for origin-auth handshake (see `docs/system/EDGE_BOT_DEFENSE.md` and `app/middleware/guards.py`)
...
- **GOOGLE_ADS_CONVERSION_ID** → Optional Google Ads conversion ID (`AW-...`) that enables direct gtag conversion routing for Ads in the shared layout/onboarding success flow (see `app/config.py`, `app/config_schema_parts/operations.py`, and `app/templates/onboarding/welcome.html`)
- **GOOGLE_ADS_PURCHASE_CONVERSION_LABEL** → Optional Google Ads purchase conversion label paired with `GOOGLE_ADS_CONVERSION_ID` to build `send_to` for direct Ads purchase conversions (see `app/config.py`, `app/config_schema_parts/operations.py`, and `app/templates/onboarding/welcome.html`)
...
- **Branch Coverage Bridge (2026-03-20 boundary stream)** → Temporary dictionary bridge entry listing all branch-touched app Python files so documentation guard path coverage remains complete while canonical terms are incrementally consolidated (see `app/blueprints/core/routes.py`, `app/blueprints/developer/system_roles.py`, `app/blueprints/organization/routes.py`, `app/blueprints/products/product_variants.py`, `app/blueprints/products/products.py`, `app/blueprints/settings/routes.py`, `app/blueprints/timers/routes.py`, `app/services/admin_debug_service.py`, `app/services/admin_organization_service.py`, `app/services/affiliate_service.py`, `app/services/api_bootstrap_service.py`, `app/services/app_dashboard_service.py`, `app/services/auth_account_service.py`, `app/services/auth_login_route_service.py`, `app/services/auth_permission_route_service.py`, `app/services/batch_finish_route_service.py`, `app/services/batch_preferences_service.py`, `app/services/batch_start_service.py`, `app/services/billing/subscription_webhook_service.py`, `app/services/container_unit_mismatch_service.py`, `app/services/conversion_route_service.py`, `app/services/developer/__init__.py`, `app/services/developer/addon_service.py`, `app/services/developer/dashboard_service.py`, `app/services/developer/debug_permission_service.py`, `app/services/developer/global_item_route_service.py`, `app/services/developer/integration_admin_service.py`, `app/services/developer/product_category_service.py`, `app/services/developer/reference_route_service.py`, `app/services/developer/subscription_tier_route_service.py`, `app/services/developer/system_role_service.py`, `app/services/drawer_resolution_service.py`, `app/services/drawers/payloads.py`, `app/services/expiration_data_service.py`, `app/services/export_recipe_service.py`, `app/services/global_library_view_service.py`, `app/services/global_link_drawer_service.py`, `app/services/ingredient_route_service.py`, `app/services/inventory_route_service.py`, `app/services/product_inventory_route_service.py`, `app/services/product_route_service.py`, `app/services/product_variant_route_service.py`, `app/services/production_planning_debug_service.py`, `app/services/public_catalog_service.py`, `app/services/recipe_ajax_service.py`, `app/services/recipe_create_view_service.py`, `app/services/recipe_form_parsing_service.py`, `app/services/recipe_form_service.py`, `app/services/recipe_library_view_service.py`, `app/services/recipe_manage_view_service.py`, `app/services/reservation_view_service.py`, `app/services/retention_service.py`, `app/services/settings_route_service.py`, `app/services/sku_route_service.py`, `app/services/tag_manager_service.py`, `app/services/timer_access_service.py`, `app/services/unit_conversion/drawer_errors.py`, `app/services/user_invite_service.py`, `app/services/user_preferences_service.py`, `app/services/whop_auth_service.py`, `app/utils/permissions.py`).
