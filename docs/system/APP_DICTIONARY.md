...
- **Legacy Alert Compatibility Shim (Retired)** → Prior runtime shim that routed legacy `alert(...)` calls through `window.showAlert`; retired after source alert callsites were migrated to direct shared-helper usage (see `app/static/js/main.js` and `docs/todo/TEMP_ALERT_NOTIFICATION_CHECKLIST.md`)
...
- **Notification Standardization Phase 8 (ALRT-088)** → Legacy-alert retirement pass that migrated remaining source-template native `alert(...)` usage to `window.showAlert(...)` and retired `main.js` alert-shim installation (see `app/templates/`, `app/static/js/main.js`, and `docs/todo/TEMP_ALERT_NOTIFICATION_CHECKLIST.md`)
