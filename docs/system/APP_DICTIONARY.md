...
- **Security Middleware** → Request-layer enforcer for permission/bot checks, billing decision application, and optional edge-origin shared-header enforcement to reject direct-to-origin bypass requests when enabled (see `app/middleware.py`)
...
- **ENFORCE_EDGE_ORIGIN_AUTH** → Security toggle that enables origin-side validation of an edge-injected shared header before serving non-exempt paths (see `app/config_schema_parts/security.py` and `app/middleware.py`)
- **EDGE_ORIGIN_AUTH_HEADER** → Configured request-header name expected from the edge/WAF when origin-auth enforcement is enabled (see `app/config_schema_parts/security.py` and `app/middleware.py`)
- **EDGE_ORIGIN_AUTH_SECRET** → Shared secret value compared against the configured edge header to validate edge-to-origin request authenticity (see `app/config_schema_parts/security.py` and `app/middleware.py`)
- **EDGE_ORIGIN_AUTH_EXEMPT_PATHS** → Comma-separated exact/prefix path rules excluded from edge-origin auth checks (health and other explicitly exempt endpoints) (see `app/config_schema_parts/security.py` and `app/middleware.py`)
- **X-Edge-Origin-Auth** → Default edge-injected request-header name used by Cloudflare transform rules and middleware validation for origin-auth handshake (see `docs/system/EDGE_BOT_DEFENSE.md` and `app/middleware.py`)
