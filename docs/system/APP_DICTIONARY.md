...
- **Legacy Alert Compatibility Shim** → Runtime shim that routes legacy `alert(...)` calls through `window.showAlert` so existing callsites adopt standardized in-app notification rendering without blocking browser popups (see `app/static/js/main.js`)
