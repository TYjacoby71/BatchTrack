...
- **Forced Verification Modal (Legacy Unverified Accounts)** → Resend-verification UI auto-opens a blocking modal when login is locked for accounts that exceeded the verification grace window, confirming a fresh verification send (or send failure) and next steps (see `app/templates/pages/auth/resend_verification.html` and `app/blueprints/auth/login_routes.py`)
