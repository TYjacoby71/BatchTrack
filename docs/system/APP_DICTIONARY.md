...
- **Soap Tool Dist Asset Mapping** → Fingerprinted soap bundle output and manifest mapping used by `static_asset(...)` resolution to serve the refactored runtime modules (see `app/static/dist/js/tools/soaps/soap_tool_bundle_entry-QRFF44Q2.js` and `app/static/dist/manifest.json`)
