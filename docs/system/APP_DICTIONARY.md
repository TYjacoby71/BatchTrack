...
- **Declarative Confirm Attributes (`data-confirm-*`)** → HTML-level confirmation convention where forms/anchors provide confirmation metadata and shared runtime handlers route blocking decisions through global confirmation modals (see `app/static/js/main.js` and templates under `app/templates/`)
