...
- **Automation**: use `flask community-scout-scheduler --interval-minutes 1440 --window-start 02:00 --window-end 04:00 --no-primary` to run the job automatically inside a narrow off-hours window. The scheduler keeps a rolling timer and skips runs if the replica is offline when `--no-primary` is set.
...
- **Dashboard alerts**: the developer dashboard now surfaces replica/health issues so operations can react before the job falls back to the primary writer.
